...
void get_ddr_info() {
    ...
    if ((fp = popen(command, "r")) != NULL) {
        ...
        while (fgets(buf, sizeof(buf), fp) != NULL) {
            ...
            // First find the line where total is located,
            // then parse out the specific name of the bandwidth,
            // and separate the string with spaces
            q = strstr(buf, "total");
            if (q != NULL) {
                is_last = 1;
                ...
                while (token != NULL) {
                    if (!is_first)
                        ddr_info_num++;
                    memcpy(ddr_info[i++].name, token, sizeof(token));
                    token = strtok(NULL, " ");
...
void* get_ddr_info_thread(void *arg) {
    while (1) {
        get_ddr_info();
        usleep(500 * 1000);
        /* if cancel should exit */
...
u32 monitor_add_ddr_load() {
    ...
    pthread_create(&ddr_info_thread, NULL, get_ddr_info_thread, NULL);
    return 0;
}
