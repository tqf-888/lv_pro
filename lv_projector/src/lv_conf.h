 ...
 *0: to disable caching*/
/*
 * 歌手页 2x4 = 8 张，开启 ARTIST_PROBE_PAGES=2 后，UI 一次会同时
 * lv_img_set_src 到当前页 + 下一页 = 16 张。这里给 16 槽，让硬件 JPG 解码出的
 * RGB 缓冲被 cache 持有，滑到下一页时直接 blit，省掉重新解码 + 文件重新打开。
 *
 * 安全前提（必须）：
 *   lvgl/src/extra/libs/sjpg/lv_sjpg.c 的 decoder_open 已经改成把 sunxijpgd 单例
 *   生产的 mRGBData detach 出来，dsc->img_data 指向独立缓冲；decoder_close 用
 *   JpegDecoderFreeFrameBuffer 自己释放。否则 cache > 0 时多 dsc 共享单例
 *   缓冲，会 use-after-free 必崩。
 *
 * 代价（LV_COLOR_DEPTH=32 时）：每张约 cell_w*cell_h*4，160*210*4 ≈ 131 KB，
 * 16 张约 2.1 MB。如果跨页面/别的地方也用 jpg，cache 可能不够（旧的会被
 * LRU 踢掉再解一次），可视情况再调大。
 */
