...
#define FILE_NAME_MAXT_LEN 64
#define FILE_PATH_MAXT_LEN 128
...
static void list_del_cur_node(list_head_t *head)
{
    ...
    if (head->first_node == head->current_node) {
        ...
        if (head->cur_index == head->total) head->cur_index = 1;
        tmp_node = head->current_node;
        head->current_node = tmp_node->next;
        tmp_node->pre->next = tmp_node->next;
        tmp_node->next->pre = tmp_node->pre;
...
static int list_append(list_head_t *head, char *path, char *name, media_file_type_t type)
{
    ...
    strncpy(new_node->name, name, FILE_NAME_MAXT_LEN);
    new_node->type = type;
...
static int list_skip_to_head(list_head_t *head)
{
    if (!head) {
        printf("list head is null!\n");
        return -1;
...
static int list_skip_to_index(list_head_t *head, int index)
{
    int i = 0;
    ...
    while (index != head->cur_index) {
        next_node = list_get_next_node(head);
        if (!next_node) {
