...
void glist_free_node(glist *list) { free(list); }
...
glist *glist_find_custom(glist *list, void *data, GCompareDataFunc func)
{
    ...
    while (list) {
        if (!func(list->data, data, NULL)) return list;
        list = list->next;
    }
...
glist *glist_find_custom_from_the_back(glist *list, void *data, GCompareDataFunc func)
{
    if (!func) return list;
...
/**
 * glist_reverse:
 * reverse "1st element->2nd element->.....->nth element" to "nth element->.....->2nd element->1st
 * element"
 *
 * Returns: new list
 */
...
glist *glist_insert(glist *list, void *data, int position)
{
    ...
    if (!tmp_list) return glist_append(list, data);
...
static glist *glist_sort_merge(glist *l1, glist *l2, GCompareDataFunc compare_func, void *user_data)
{
    glist list, *l, *lprev;
    int cmp;
...
glist *glist_sort(glist *list, GCompareDataFunc compare_func, void *user_data)
{
    ...
    // below is the insertion method. 1. get the last list, 2. insert the last list to the previous
    // sorted list.
    /***************************************/
    ...
    return glist_sort_merge(glist_sort(list, compare_func, user_data),
                            glist_sort(l2, compare_func, user_data), compare_func, user_data);
}
...
int atomic_int_exchange_and_add(pthread_mutex_t *atomic_mutex, volatile int *atomic, int val)
{
    int result;

    if (!atomic_mutex) return *atomic + val;
...
static void ptr_array_foreach(PtrArray *array, GFunc func, void *user_data)
{
    int i;
...
PtrArray *ptr_array_ref(PtrArray *array)
{
    ...
    if (!array || rarray->ref_count <= 0) return array;
...
void ptr_array_unref(PtrArray *array)
{
    ...
    if (!array || rarray->ref_count <= 0) return;
