...
static int is_directory_empty(const char *dirpath) {
    DIR *dir;
    struct dirent *entry;
    ...
    // If "." or ".." is the first entry, continue to the next one
    if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0) {
        entry = readdir(dir);
        if (entry == NULL) {
            closedir(dir);
            return 1; // Directory is empty (only "." and "..")
        }
    }

    // Close the directory and return 0 (Directory is not empty)
    closedir(dir);
    return 0;
}
...
static int DiskManager_Clean_MountPoint(void) {
    ...
    for (;Index < sizeof(UsbMountPoint)/sizeof(UsbMountPoint[0]); Index++) {
        if (umount(UsbMountPoint[Index].MountPoint) == 0) {
            if (UsbMountPoint[Index].NeedCreate_Clean && is_directory_empty(UsbMountPoint[Index].MountPoint) == 1) {
                char Cmd[64] = {0};
    ...
    for (;Index < sizeof(SdcardMountPoint)/sizeof(SdcardMountPoint[0]); Index++) {
        if (umount(SdcardMountPoint[Index].MountPoint) == 0) {
            if (SdcardMountPoint[Index].NeedCreate_Clean && is_directory_empty(SdcardMountPoint[Index].MountPoint) == 1) {
                char Cmd[64] = {0};
...
static int DiskManager_MountDevice(DiskInfo_t *DeviceInfo, int bootcheck) {
    ...
    if (DiskManager_Check_DiskMountStatus(DeviceInfo) == 0)
    {
        ...
        if (DiskManager_GetMountPoint(DeviceInfo) != 0) {
            printf("DiskManager_GetMountPoint fail\n");
            return -1;
        }
        for (Index = 0; Index < sizeof(FileSystemType)/sizeof(FileSystemType[0]); Index++) {
            if (0) {
                ...
                char command[128] = {0};
                snprintf(command, 128, "mount -t %s -o utf8 %s %s", FileSystemType[Index], DeviceInfo->DeviceName, DeviceInfo->MountPoint);
//              printf("command:%s\n", command);
                ret = system(command);
            }
            if (ret != 0) {
...
//              printf("mount fail:%s\n", strerror(errno));
        ...
        if (DiskManager_Item == NULL || NULL == hotplug_message) {
            ...
            if (umount(DeviceInfo->MountPoint) == 0)
                DiskManager_Free_MountPoint(DeviceInfo);
            printf("malloc fail\n");
            return -1;
        }
        ...
        printf("%s already mount", DeviceInfo->DeviceName);
        printf("mountinfo:");
        system("mount");
    }
    return 0;
...
static void uevent_init(void)
{
    ...
    if (uevent_fd < 0) {
        printf("uevent_init: uevent_open_socket failed\n");
        return;
    }
    fcntl(uevent_fd, F_SETFL, O_NONBLOCK);
    if (healthd_register_event(uevent_fd, uevent_event)) {
        printf("register for uevent events failed\n");
        return ;
    }
