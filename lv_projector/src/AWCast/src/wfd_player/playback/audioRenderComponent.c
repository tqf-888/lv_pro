...
int WFD_AudioRenderCompDestroy(AudioRenderComp* p)
{
...
int WFD_AudioRenderCompStart(AudioRenderComp* p)
{
    return BaseCompStart(&p->base, NULL, NULL);
}

int WFD_AudioRenderCompStop(AudioRenderComp* p)
{
    return BaseCompStop(&p->base, NULL, NULL);
}

int WFD_AudioRenderCompPause(AudioRenderComp* p)
{
    return BaseCompPause(&p->base, NULL, NULL);
}

int WFD_AudioRenderCompReset(AudioRenderComp* p)
{
    return BaseCompReset(&p->base, 0, NULL, NULL);
}

int WFD_AudioRenderCompSetEOS(AudioRenderComp* p)
{
    return BaseCompSetEos(&p->base, NULL, NULL);
}

int WFD_AudioRenderCompSetCallback(AudioRenderComp* p, PlayerCallback callback, void* pUserData)
{
    p->callback  = callback;
...
int WFD_AudioRenderCompSetTimer(AudioRenderComp* p, AvTimer* timer)
{
    p->pAvTimer = timer;
    return 0;
}

int WFD_AudioRenderCompSetAudioSink(AudioRenderComp* p, SoundCtrl* pAudioSink)
{
    sem_t replySem;
...
int WFD_AudioRenderCompSetDecodeComp(AudioRenderComp* p, AudioDecComp* d)
{
    p->pDecComp = d;
    return 0;
}
...
static void handleStart(AwMessage *msg, void *arg)
{
    ...
    else
    {
        ...
        if(p->pSoundCtrl && p->threadCtx->bFirstFrameSend == 1)
        {
...
static int initSoundDevice(AudioRenderComp *p)
{
    ...
    do {
        ret = WFD_AudioDecCompGetAudioSampleRate(p->pDecComp, &threadCtx->nSampleRate,
...
static inline int requestPcmData(AudioRenderComp *p)
{
    ...
    do
    {
        ...
        ret = WFD_AudioDecCompRequestPcmData(p->pDecComp, &pPcmData,
...
static inline int checkSampleRate(AudioRenderComp *p)
{
    ...
    ret = WFD_AudioDecCompGetAudioSampleRate(p->pDecComp, &nNewSampleRate,
...
static int startSoundDevice(AudioRenderComp *p)
{
    ...
    {
        WFD_AudioDecCompReleasePcmData(p->pDecComp, threadCtx->nPcmDataLen);
...
static inline int notifyAudioPts(AudioRenderComp *p)
{
    ...
    if(ret == TIMER_DROP_AUDIO_DATA)
    {
        WFD_AudioDecCompReleasePcmData(p->pDecComp, p->threadCtx->nPcmDataLen);
        BaseCompContinue(&p->base);
        return -1;
    }
...
static inline int writeToSoundDevice(AudioRenderComp *p)
{
    ...
    while(nPcmDataLen > 0)
    {
        ...
        if(nWritten > 0)
        {
            ...
            pPcmData    += nWritten;
            WFD_AudioDecCompReleasePcmData(p->pDecComp, nWritten);
            usleep(1000);
        }
        else
        {
            ...
            {
                WFD_AudioDecCompReleasePcmData(p->pDecComp, nPcmDataLen);
                return -1;
            }
        }
    }
