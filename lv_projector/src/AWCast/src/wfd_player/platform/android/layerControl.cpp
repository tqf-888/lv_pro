...
static int setLayerParam(LayerCtrlContext* lc)
{
    ...
    //*add for interlaced detection and denoise
	char idet_value[PROP_VALUE_MAX] = {0};
    char hqdn3d_value[PROP_VALUE_MAX] = {0};
	property_get("media.idet", idet_value, "0");
    property_get("media.denoise", hqdn3d_value, "0");

	if(atoi(idet_value) == 1 || atoi(hqdn3d_value) == 1)
	{
		logd("IDET_TEST or HQDN3D,lc->nUsage open !");
                                                  ...
                                                  int bInitFlag)
{
    ...
    for(i = 0; i < lc->nGpuBufferCount; i++)
    {
        ...
        else if(lc->mGpuBufferInfo[i].pWindowBuf == pWindowBuf)
        {
            ...
            if (ion_import(lc->ionFd, lc->mGpuBufferInfo[i].nBufFd, &(data.handle))) {
                loge("%s: +++ ion_import error, --- share_fd=%d", __func__, lc->mGpuBufferInfo[i].nBufFd);
...
static int __LayerQueueBuffer(LayerCtrl* l,VideoPicture* pInPicture, int bValid)
{
    ...
    logv("LayerQueueBuffer: cur time   %12lld   pts   %12lld   ",
            GetCurrentTime(), pInPicture->nPts);
    ANativeWindowBuffer* pWindowBuf = NULL;
    int               i      = 0;
    char*             pBuf   = NULL;
    int               nBufId = -1;
...
#endif
		int ptsSecs = (int)(pInPicture->nPts/1000000);
		if (lc->ptsSecs != ptsSecs)
		{
			lc->ptsSecs = ptsSecs;
			logd("pic_trace: (%.3f) do render", pInPicture->nPts/1000000.0);
		}
        lc->pNativeWindow->queueBuffer_DEPRECATED(lc->pNativeWindow, pWindowBuf);
    }
    else
