                                    ...
                                    size_t nMessageSize)
{
    ...
    if(mqCtx == NULL)
    {
        CDX_LOGE("%s, allocate memory fail.", pName);
        return NULL;
    }
    ...
    if(mqCtx->Nodes == NULL)
    {
        CDX_LOGE("%s, allocate memory for message nodes fail.", mqCtx->pName);
        if(mqCtx->pName != NULL)
            free(mqCtx->pName);
        free(mqCtx);
        return NULL;
...
int WFDMessageQueuePostMessage(WFDMessageQueue* mq, WFDMessage* m)
{
    ...
    {
        CDX_LOGE("%s, message count exceed, current message count = %d, max message count = %d",
...
int WFDMessageQueueTryGetMessage(WFDMessageQueue* mq, WFDMessage* m, int64_t timeout)
{
    ...
    if(mqCtx->nCount <= 0)
    {
        CDX_LOGV("%s, no message.", mqCtx->pName);
        pthread_mutex_unlock(&mqCtx->mutex);
        return -1;
    }
...
int WFDMessageQueueFlush(WFDMessageQueue* mq)
{
    ...
    CDX_LOGI("%s, flush messages.", mqCtx->pName);
