...
static char *ctrl_iface_dir = CONFIG_CTRL_IFACE_DIR;
...
{
	CDX_LOGD("wpa_cli [-p<path to ctrl sockets>] [-i<ifname>] [-hvB] "
	       "[-a<action file>] \\\n"
	       ...
	       "  default path: see /etc/miracast.cfg\n"
...
static int wpa_cli_open_connection(const char *ifname, int attach)
{
	...
	if (mon_conn) {
		if (wpa_ctrl_attach(mon_conn) == 0) {
			...
			CDX_LOGD("Warning: Failed to attach to "
			       "wpa_supplicant.\n");
...
static int _wpa_ctrl_command(struct wpa_ctrl *ctrl, char *cmd, int print)
{
	...
	if (ctrl_conn == NULL) {
		CDX_LOGD("Not connected to wpa_supplicant - command dropped.\n");
		return -1;
	}
	if (ifname_prefix) {
	...
	ret = wpa_ctrl_request(ctrl, cmd, os_strlen(cmd), buf, &len,
			       wpa_cli_msg_cb);
	if (ret == -2) {
		CDX_LOGD("'%s' command timed out.\n", cmd);
		return -2;
	} else if (ret < 0) {
		CDX_LOGD("'%s' command failed.\n", cmd);
		return -1;
	}
	if (print) {
		buf[len] = '\0';
		CDX_LOGD("wpa_ctrl_request %s return %s", cmd,  buf);
		       ...
		       int argc, char *argv[])
{
	char buf[4096];
	if (argc < min_args) {
		CDX_LOGD("Invalid %s command - at least %d argument%s "
		       "required.\n", cmd, min_args,
		       min_args > 1 ? "s are" : " is");
		return -1;
	}
...
static int wpa_cli_cmd_bss_flush(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (os_snprintf_error(sizeof(cmd), res)) {
		CDX_LOGD("Too long BSS_FLUSH command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}
...
{
	if (argc == 0) {
		CDX_LOGD("Invalid WPS_PIN command: need one or two arguments:\n"
					...
					char *argv[])
{
	...
	if (argc != 1) {
		CDX_LOGD("Invalid 'wps_nfc_tag_read' command - one argument "
		       "is required.\n");
		return -1;
	}
...
static int wpa_cli_cmd_wps_reg(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc == 2)
		...
		CDX_LOGD("Invalid WPS_REG command: need two arguments:\n"
		       "- BSSID of the target AP\n"
		       "- AP PIN\n");
		CDX_LOGD("Alternatively, six arguments can be used to "
		       "reconfigure the AP:\n"
		       "- BSSID of the target AP\n"
		       "- AP PIN\n"
		       "- new SSID\n"
	...
	if (os_snprintf_error(sizeof(cmd), res)) {
		CDX_LOGD("Too long WPS_REG command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}
...
{
	if (argc < 2) {
		CDX_LOGD("Invalid WPS_ER_PIN command: need at least two "
		       "arguments:\n"
...
{
	if (argc != 2) {
		CDX_LOGD("Invalid WPS_ER_LEARN command: need two arguments:\n"
...
{
	if (argc != 2) {
		CDX_LOGD("Invalid WPS_ER_SET_CONFIG command: need two "
		       "arguments:\n"
		       "- UUID: specify which AP to use\n"
				     ...
				     char *argv[])
{
	...
	if (argc == 5 || argc == 6) {
		...
		CDX_LOGD("Invalid WPS_ER_CONFIG command: need six arguments:\n"
		       "- AP UUID\n"
		       "- AP PIN\n"
		       "- new SSID\n"
		       "- new auth (OPEN, WPAPSK, WPA2PSK)\n"
	...
	if (os_snprintf_error(sizeof(cmd), res)) {
		CDX_LOGD("Too long WPS_ER_CONFIG command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}
...
{
	if (argc != 2) {
		CDX_LOGD("Invalid WPS_ER_NFC_CONFIG_TOKEN command: need two "
		       "arguments:\n"
...
static int wpa_cli_cmd_identity(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Invalid IDENTITY command: needs two arguments "
	...
	if (os_snprintf_error(end - pos, ret)) {
		CDX_LOGD("Too long IDENTITY command.\n");
		return -1;
	}
	pos += ret;
	for (i = 2; i < argc; i++) {
...
static int wpa_cli_cmd_password(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Invalid PASSWORD command: needs two arguments "
	...
	ret = os_snprintf(pos, end - pos, WPA_CTRL_RSP "PASSWORD-%s:%s",
			  argv[0], argv[1]);
	if (os_snprintf_error(end - pos, ret)) {
		CDX_LOGD("Too long PASSWORD command.\n");
		return -1;
	}
	pos += ret;
	for (i = 2; i < argc; i++) {
				    ...
				    char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Invalid NEW_PASSWORD command: needs two arguments "
		       "(network id and password)\n");
		return -1;
	}
	...
	ret = os_snprintf(pos, end - pos, WPA_CTRL_RSP "NEW_PASSWORD-%s:%s",
			  argv[0], argv[1]);
	if (os_snprintf_error(end - pos, ret)) {
		CDX_LOGD("Too long NEW_PASSWORD command.\n");
		return -1;
	}
	pos += ret;
	for (i = 2; i < argc; i++) {
...
static int wpa_cli_cmd_pin(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Invalid PIN command: needs two arguments "
		       "(network id and pin)\n");
		return -1;
	}
...
static int wpa_cli_cmd_otp(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Invalid OTP command: needs two arguments (network "
		       "id and password)\n");
		return -1;
	}
...
static int wpa_cli_cmd_sim(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Invalid SIM command: needs two arguments "
				  ...
				  char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Invalid PASSPHRASE command: needs two arguments "
	...
	ret = os_snprintf(pos, end - pos, WPA_CTRL_RSP "PASSPHRASE-%s:%s",
			  argv[0], argv[1]);
	if (os_snprintf_error(end - pos, ret)) {
		CDX_LOGD("Too long PASSPHRASE command.\n");
		return -1;
	}
	pos += ret;
	for (i = 2; i < argc; i++) {
...
{
	if (argc < 2) {
		CDX_LOGD("Invalid BSSID command: needs two arguments (network "
		       "id and BSSID)\n");
		return -1;
	}
...
{
	CDX_LOGD("set_network variables:\n"
				   ...
				   char *argv[])
{
	...
	if (argc < 3) {
		CDX_LOGD("Invalid SET_NETWORK command: needs three arguments\n"
				   ...
				   char *argv[])
{
	...
	if (argc != 2) {
		CDX_LOGD("Invalid GET_NETWORK command: needs two arguments\n"
				   ...
				   char *argv[])
{
	...
	if (argc < 3) {
		CDX_LOGD("Invalid DUP_NETWORK command: needs three arguments\n"
...
{
	if (argc != 3) {
		CDX_LOGD("Invalid SET_CRED command: needs three arguments\n"
...
{
	if (argc != 2) {
		CDX_LOGD("Invalid GET_CRED command: needs two arguments\n"
...
{
	if (argc < 1 || argc > 2) {
		CDX_LOGD("Invalid GET_CAPABILITY command: need either one or "
		       "two arguments\n");
		return -1;
	}

	if ((argc == 2) && os_strcmp(argv[1], "strict") != 0) {
		CDX_LOGD("Invalid GET_CAPABILITY command: second argument, "
...
static int wpa_cli_cmd_interface(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (!ctrl_ifname) {
		CDX_LOGD("Failed to allocate memory\n");
		return 0;
	}

	if (wpa_cli_open_connection(ctrl_ifname, 1) == 0) {
		CDX_LOGD("Connected to interface '%s.\n", ctrl_ifname);
	} else {
		CDX_LOGD("Could not connect to interface '%s' - re-trying\n",
		       ctrl_ifname);
	}
	return 0;
}
				     ...
				     char *argv[])
{
	...
	if (argc < 1) {
		CDX_LOGD("Invalid INTERFACE_ADD command: needs at least one "
				...
				char *addr, size_t addr_len)
{
	...
	if (ctrl_conn == NULL) {
		CDX_LOGD("Not connected to hostapd - command dropped.\n");
		return -1;
	}
	if (ifname_prefix) {
		os_snprintf(buf, sizeof(buf), "IFNAME=%s %s",
...
{
	if (argc != 2 && argc != 3) {
		CDX_LOGD("Invalid P2P_PROV_DISC command: needs at least "
					 ...
					 char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Invalid P2P_SERV_DISC_REQ command: needs two "
					  ...
					  char *argv[])
{
	...
	if (argc != 4) {
		CDX_LOGD("Invalid P2P_SERV_DISC_RESP command: needs four "
...
{
	if (argc < 3) {
		CDX_LOGD("Invalid P2P_SERVICE_ADD command: needs 3-6 arguments\n");
		return -1;
	}
...
{
	if (argc < 5 || argc > 6) {
		CDX_LOGD("Invalid P2P_SERVICE_REP command: needs 5-6 "
		       "arguments\n");
		return -1;
	}
				       ...
				       char *argv[])
{
	...
	if (argc != 2 && argc != 3) {
		CDX_LOGD("Invalid P2P_SERVICE_DEL command: needs two or three "
		       "arguments\n");
		return -1;
	}
					...
					char *argv[])
{
	if (argc != 0 && argc != 2 && argc != 4) {
		CDX_LOGD("Invalid P2P_PRESENCE_REQ command: needs two arguments "
...
{
	if (argc != 0 && argc != 2) {
		CDX_LOGD("Invalid P2P_EXT_LISTEN command: needs two arguments "
				       ...
				       char *argv[])
{
	...
	if (argc != 1 && argc != 2) {
		CDX_LOGD("Invalid WFD_SUBELEM_SET command: needs one or two "
				       ...
				       char *argv[])
{
	...
	if (argc != 1) {
		CDX_LOGD("Invalid WFD_SUBELEM_GET command: needs one "
		       "argument (subelem)\n");
		return -1;
	}
					       ...
					       char *argv[])
{
	...
	if (argc == 0) {
		CDX_LOGD("Command needs one or two arguments (dst mac addr and "
		       "optional home realm)\n");
		return -1;
	}
					 ...
					 char *argv[])
{
	...
	if (argc < 2) {
		CDX_LOGD("Command needs two arguments (dst mac addr and "
		       "icon name)\n");
		return -1;
	}
...
static int wpa_request(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (count > 1) {
		CDX_LOGD("Ambiguous command '%s'; possible commands:", argv[0]);
		cmd = wpa_cli_commands;
		while (cmd->cmd) {
...
static int wpa_cli_cmd_with_reply(const char *cmd, char *reply, size_t *reply_len)
{
	int ret = -1;

	if (ctrl_conn == NULL) {
		CDX_LOGD("Not connected to wpa_supplicant - \"%s\" command dropped.\n", cmd);
		return -1;
	}
...
static void wpa_cli_action_process(const char *msg)
{
	...
	if (str_starts(pos, WPA_EVENT_CONNECTED)) {
		...
		if (wfdP2PCB)
		{
			wfdP2PCB(WFD_CB_GROUP_STARTED, pos, 0);
		}
		int ret = wpa_cli_exec("/usr/bin/p2p_action.sh", ifname, pos);
		CDX_LOGD("/usr/bin/p2p_action.sh ret: %d\n", ret);
		...
		if (argc < 8) {
			CDX_LOGW("wrong P2P-GROUP-STARTED");
			return;
		}
		char *go_or_client = argv[2];
		CDX_LOGD("P2P-GROUP-STARTED: %s\n", go_or_client);
		char *deviceAddress = NULL;
		char go_dev_addr[32] = {0};
		if (os_strcmp(go_or_client, "client") == 0) {
			pos = os_strstr(pos, "go_dev_addr=");
			pos += 12;
			if (os_strlen(pos) < 17)
			{
				CDX_LOGW("invalid msg '%s'", copy);
				os_free(copy);
				return;
			}
			os_memcpy(go_dev_addr, pos, 17); /* xx:xx:xx:xx:xx:xx */
			CDX_LOGD("P2P-GROUP-STARTED: go_dev_addr: %s", go_dev_addr);
			...
			CDX_LOGD("wrong P2P-GROUP-STARTED\n");
		}
		...
		argc = tokenize_cmd(copy, argv);
		if (argc < 2) {
			CDX_LOGD("wrong P2P-GO-NEG-REQUEST\n");
			return;
		}
		char *deviceAddress = argv[1];
		CDX_LOGD("P2P-GO-NEG-REQUEST: %s\n", deviceAddress);
		if (wfdP2PCB) {
			CDX_LOGD("%s:%d: \n", __func__, __LINE__);
			wfdP2PCB(WFD_CB_GO_NEG_REQUEST, NULL, 0);
		}
		int devPasswdId = 0;
		if (argc > 2) {
			pos = os_strstr(argv[2], "dev_passwd_id=");
			devPasswdId = atoi(pos + 14);
			CDX_LOGD("P2P-GO-NEG-REQUEST: devPasswdId=%d\n", devPasswdId);
		}
		if (argc > 3) {
			pos = os_strstr(argv[3], "go_intent=");
			int goIntent = atoi(pos + 10);
			CDX_LOGD("P2P-GO-NEG-REQUEST: goIntent=%d\n", goIntent);
		}
		...
		if (ret >= 0 && os_strncmp(reply, "PONG", 4) != 0 && os_strncmp(reply, "FAIL", 4) != 0
			&& os_strncmp(reply, "OK", 2) != 0) {
			CDX_LOGD("%s: will parse group_capab\n", p2p_peer_cmd);
		...
		if (end && (unsigned int) (end - pos) < sizeof(peer_dev_addr)) {
			os_memcpy(peer_dev_addr, pos, end - pos);
			peer_dev_addr[end - pos] = '\0';
			CDX_LOGD("P2P-INVITATION-ACCEPTED: %s\n", peer_dev_addr);
		}

		if (wfdP2PCB) wfdP2PCB(WFD_CB_INVITATION_ACCEPTED, NULL, 0);
	} else if (str_starts(pos, WPA_EVENT_TERMINATING)) {
		CDX_LOGD("wpa_supplicant is terminating - stop monitoring\n");
		wpa_cli_quit = 1;
	} else if (str_starts(pos, "CTRL-EVENT-")) {
	} else if (str_starts(pos, P2P_EVENT_DEVICE_LOST)) {
		CDX_LOGD("P2P_EVENT_DEVICE_LOST: %s\n", pos);
        if (os_strstr(pos, peer_dev_addr)) {
		    CDX_LOGD("P2P_EVENT_DEVICE_LOST is corresponding to the connecting phone,"
                     "not the last phone. addr: %s\n", peer_dev_addr);
            //according to P2P_EVENT_GROUP_REMOVED to disconnect miracast
            //if (wfdP2PCB) wfdP2PCB(WFD_CB_DISCONNECTED, NULL, 0);
        }
	} else if (str_starts(pos, P2P_EVENT_DEVICE_FOUND)) {
		CDX_LOGD("P2P_EVENT_DEVICE_FOUND: %s\n", pos);
		if (wfdP2PCB) wfdP2PCB(WFD_CB_DEVICE_FOUND, NULL, 0);
	} else {
		CDX_LOGD("%s: %s, ignore\n", ifname, pos);
	}
}
...
static void wpa_cli_reconnect(void)
{
	...
	if (interactive) {
		edit_clear_line();
		CDX_LOGD("\rConnection to wpa_supplicant re-established\n");
		edit_redraw();
	}
}
...
static void cli_event(const char *str)
{
	...
	if (str_starts(start, P2P_EVENT_DEVICE_FOUND)) {
		...
		if (wfdP2PCB) {
			wfdP2PCB(WFD_CB_DEVICE_FOUND, NULL, 0);
		}
		return;
	}
	...
	if (str_starts(start, P2P_EVENT_DEVICE_LOST)) {
		...
		if (wfdP2PCB) {
			wfdP2PCB(WFD_CB_DISCONNECTED, NULL, 0);
		}
		return;
	}
...
static int check_terminating(const char *msg)
{
	...
	if (str_starts(pos, WPA_EVENT_TERMINATING) && ctrl_conn) {
		edit_clear_line();
		CDX_LOGD("\rConnection to wpa_supplicant lost - trying to "
		       "reconnect\n");
		edit_redraw();
		wpa_cli_attached = 0;
		wpa_cli_close_connection();
...
static void wpa_cli_recv_pending(struct wpa_ctrl *ctrl, int action_monitor)
{
	...
	while (wpa_ctrl_pending(ctrl) > 0) {
		...
		if (wpa_ctrl_recv(ctrl, buf, &len) == 0) {
			...
			CDX_LOGD("Could not read pending message.\n");
			break;
		}
	}

	if (wpa_ctrl_pending(ctrl) < 0) {
		CDX_LOGD("Connection to wpa_supplicant lost - trying to "
		       "reconnect\n");
		wpa_cli_reconnect();
	}
}
...
static void try_connection(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_cli_open_connection(ctrl_ifname, 1)) {
		if (!warning_displayed) {
			CDX_LOGD("Could not connect to wpa_supplicant: "
			       "%s - re-trying\n",
	...
	if (warning_displayed)
		CDX_LOGD("Connection established.\n");
...
static void wpa_cli_action_ping(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_ctrl_request(ctrl, "PING", 4, buf, &len,
		...
		CDX_LOGD("wpa_supplicant did not reply to PING command - exiting\n");
		eloop_terminate();
		return;
	}
...
static void wpa_cli_action(struct wpa_ctrl *ctrl)
{
	...
	/* TODO: ANSI C version(?) */
	CDX_LOGD("Action processing not supported in ANSI C build.\n");
		...
		if (interactive) {
			...
			if (mon_conn) {
				if (wpa_ctrl_attach(mon_conn) == 0) {
					...
					CDX_LOGD("Failed to open monitor "
...
{
	CDX_LOGD("CONFIG_CTRL_IFACE not defined - wpa_cli disabled\n");
	return -1;
}
#endif /* CONFIG_CTRL_IFACE */
