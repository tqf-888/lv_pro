...
if [ "$CMD" = "P2P-GROUP-STARTED" ]; then
    ...
    if [ "$4" = "client" ]; then
	if [ -f /etc/wifi/wmg_udhcpc.script ]; then
		echo "use wifimanager wmg_udhcpc.script"
		udhcpc -i $GIFNAME -s /etc/wifi/wmg_udhcpc.script
	else
		echo "use default script"
		udhcpc -i $GIFNAME
	fi
    fi
fi
...
#    ifconfig $GIFNAME down
    ...
    ps | grep $GIFNAME | grep -v grep | awk '{print $1}' | xargs kill -9
fi
