 ...
 */
#include "lv_renderer_order_song.h"
#include "cJSON.h"
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
...
static lv_order_song_adapter_t *g_order_song_default = NULL;
 ...
 * 生成批次文件路径。
 * 新逻辑优先使用带 epoch 的文件名：
 *   /tmp/order_song_batch_<epoch>_<batch>.json
 * 这样旧请求和新请求不会写到同一个文件里。
 *
 * 下面保留旧路径兜底：
 *   /tmp/order_song_batch_<batch>.json
 */
static int rs_batch_build_path(uint32_t batch_index, uint32_t request_epoch, char *buf, size_t size)
{
    ...
    if (request_epoch != 0U) {
        n = snprintf(buf, size, "/tmp/order_song_batch_%u_%u.json", request_epoch, batch_index);
        if (n >= 0 && (size_t)n < size) {
            return 0;
        }
    }

    n = snprintf(buf, size, "/tmp/order_song_batch_%u.json", batch_index);
...
/* 给歌曲行填默认文字样式 */
static void rs_fill_default_text_style(lv_order_song_item_t *it)
{
    if (it == NULL) return;
 ...
 */
static int rs_sync_batch_from_json(lv_order_song_adapter_t *adapter,
                                   ...
                                   rs_batch_sync_meta_t *meta)
{
    ...
    /*
     * 兼容两种返回结构：
     * 1. 旧版：{"result":{"data":[],"total_count":123}}
     * 2. 新版：{"code":0,"msg":"success","data":[...]}
     */
    ...
    if (result != NULL) {
        ...
        json_total_count = cJSON_GetObjectItem(root, "total_count");
    }
    ...
    /*
     * order_song 是整包一次下发。
     * 如果真实条数大于 open 时预估的 total_count，必须先把 catalog 扩大，
     * 否则后面的条目会被 start_id/end_id 上限硬截断。
     *
     * 这里 batch_count 仍然固定 1，不走分页；只扩大可容纳的总条目数。
     */
    if (batch_index == 0U && array_size > 0 && (uint32_t)array_size > adapter->total_count) {
        uint32_t new_total = (uint32_t)array_size;

        if (!lv_order_song_catalog_reset(&adapter->catalog, new_total)) {
            cJSON_Delete(root);
            return -1;
        }

        adapter->total_count = new_total;
        adapter->batch_size = new_total;
        adapter->batch_count = 1U;
        adapter->end_reached = 1U;
        adapter->last_batch_index = 0U;

        start_id = 0U;
        end_id = new_total;

        printf("[order_song] auto expand catalog: total=%u\n", new_total);
    }
    ...
    for (i = 0; i < array_size && (start_id + (uint32_t)i) < end_id; ++i) {
        ...
        /* 新接口字段名：isVip / mvId；旧接口字段名：is_vip / mv_id。 */
        if (is_vip == NULL) is_vip = cJSON_GetObjectItem(node, "isVip");
        if (mv_id == NULL) mv_id = cJSON_GetObjectItem(node, "mvId");

        lv_order_song_item_t item;
        int vip_flag = 0;
        int mv_flag = 0;
        ...
        (void)lv_order_song_catalog_set_item_by_slot(&adapter->catalog, item.slot_id, &item);
    }
...
{
    lv_order_song_adapter_t *adapter = (lv_order_song_adapter_t *)user_ctx;
    return (adapter != NULL) ? lv_order_song_catalog_count(&adapter->catalog) : 0U;
}
 ...
 * - 正在 loading 的不再重复请求
 */
static void rs_request_batch(lv_order_song_adapter_t *adapter, uint32_t batch_index)
{
    /* order_song 固定单批次，只允许请求 batch 0。 */
    if (adapter == NULL || batch_index >= adapter->batch_count || batch_index != 0U) {
        return;
    }

    if (adapter->end_reached && batch_index > adapter->last_batch_index) {
        printf("[order_song] 跳过请求: batch=%u 已超过最后一页 batch=%u\n",
               batch_index,
    ...
    /* order_song 接口无分页：外部只需要处理 batch 0，一次把整包数据落盘即可。 */
    fetch_order_song_batch(0);
}

static int rs_finish_first_batch_gate_if_needed(lv_order_song_adapter_t *adapter,
                                                uint32_t batch_index)
{
    ...
    printf("[order_song] 首批回调完成，补做最后一次reset: total=%u epoch=%u\n",
           deferred_total,
           adapter->request_epoch);

    lv_order_song_adapter_reset(adapter, deferred_total);
    return 1;
}
...
static bool rs_get_item(uint32_t index, lv_vlist_item_t *out, void *user_ctx)
{
    ...
    return lv_order_song_catalog_get_vlist_item(&adapter->catalog, index, out);
}
                             ...
                             lv_coord_t rel_y)
{
    ...
    if (!lv_order_song_catalog_get_item(&adapter->catalog, item_id, &item)) {
        return;
    }
    lv_order_song_row_style_t  *s = &adapter->view_style->row_style;
    ...
    if (item_changed != 0)
    {
        (void)lv_order_song_catalog_set_item_by_slot(&adapter->catalog, item_id, &item);
...
static int rs_pop_one_batch(lv_order_song_adapter_t *adapter,
                            uint32_t *batch_index,
                            uint8_t *success,
                            uint32_t *request_epoch)
{
...
static void rs_apply_last_page_if_needed(lv_order_song_adapter_t *adapter,
                                        uint32_t batch_index,
                                        const rs_batch_sync_meta_t *meta)
{
    ...
    if (final_total == adapter->total_count) {
        printf("[order_song] 已到最后一页: batch=%u, recv=%u, batch_size=%u, final_total=%u\n",
               batch_index,
               meta->data_count,
               adapter->batch_size,
               final_total);
     ...
     */
    (void)lv_order_song_catalog_truncate(&adapter->catalog, final_total);

    printf("[order_song] 已到最后一页: batch=%u, recv=%u, batch_size=%u, final_total=%u, json_total_count=%u%s\n",
           batch_index,
           meta->data_count,
           adapter->batch_size,
           final_total,
...
{
    lv_order_song_adapter_t *adapter = (lv_order_song_adapter_t *)timer->user_data;
    uint32_t batch_index;
    uint32_t request_epoch;
    uint8_t success;
                               ...
                               uint32_t batch_size)
{
    ...
    /*
     * order_song 的 HTTP 接口没有分页。
     * 为了最大限度复用 song 原来的 catalog/vlist/render 结构，
     * 这里直接把整份列表视为“单批次”。
     *
     * 结论：
     * - 外部虽然仍保留 batch_index 形参，但只会收到 0；
     * - 一次请求下载整包 json；
     * - UI 层其余逻辑不需要改。
     */
    (void)batch_size;
    adapter->batch_size = total_count;
    adapter->batch_count = 1U;

    if (!lv_order_song_catalog_init(&adapter->catalog, total_count)) {
        return -1;
    }
    ...
    if (adapter->batch_ready == NULL ||
        ...
        lv_order_song_adapter_stop(adapter);
        return -1;
    }
...
void lv_order_song_adapter_stop(lv_order_song_adapter_t *adapter)
{
    ...
    lv_order_song_catalog_deinit(&adapter->catalog);

    if (g_order_song_default == adapter) {
        g_order_song_default = NULL;
    }
...
lv_vlist_t *lv_order_song_adapter_create_vlist(lv_order_song_adapter_t *adapter, lv_obj_t *parent)
{
    ...
    cfg.renderer_ops = &g_lv_renderer_order_song_ops;
...
void lv_order_song_adapter_reset(lv_order_song_adapter_t *adapter, uint32_t total_count)
{
    ...
    (void)lv_order_song_catalog_reset(&adapter->catalog, total_count);
    ...
    if (adapter->first_batch_inflight) {
        ...
        printf("[order_song] reset合并: 首批仍在飞，延后到首批回调后再做 total=%u epoch=%u\n",
               total_count,
               adapter->request_epoch);
        return;
    }
...
bool lv_order_song_adapter_get_business_item(lv_order_song_adapter_t *adapter, uint32_t item_id, lv_order_song_item_t *out)
{
    ...
    return lv_order_song_catalog_get_item(&adapter->catalog, item_id, out);
}
...
void lv_order_song_adapter_toggle_selected(lv_order_song_adapter_t *adapter, uint32_t item_id)
{
    ...
    lv_order_song_catalog_set_selected(&adapter->catalog, item_id, !item.selected);
    if (adapter->vlist != NULL) {
        lv_vlist_notify_item_changed(adapter->vlist, item_id);
    }
}
...
void lv_order_song_adapter_notify_batch_ready(lv_order_song_adapter_t *adapter, uint32_t batch_index, bool success)
{
    ...
    lv_order_song_adapter_notify_batch_ready_with_epoch(adapter,
                                                       batch_index,
                                                       success,
                                                       adapter->request_epoch);
}
...
{
    return lv_order_song_adapter_get_request_epoch(g_order_song_default);
}

/* 默认 adapter 的旧接口快捷方式 */
void lv_order_song_adapter_notify_default_batch_ready(uint32_t batch_index, bool success)
{
    lv_order_song_adapter_notify_default_batch_ready_with_epoch(batch_index,
                                                               success,
                                                               lv_order_song_adapter_get_default_request_epoch());
}
...
void lv_order_song_adapter_notify_default_batch_ready_with_epoch(uint32_t batch_index,
                                                                ...
                                                                uint32_t request_epoch)
{
    lv_order_song_adapter_notify_batch_ready_with_epoch(g_order_song_default,
                                                       batch_index,
                                                       success,
                                                       request_epoch);
}
