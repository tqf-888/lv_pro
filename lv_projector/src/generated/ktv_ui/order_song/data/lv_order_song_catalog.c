...
#include "../lv_order_song_debug.h"
#include <stdlib.h>
#include <string.h>

static void rs_init_item(lv_order_song_item_t *item, uint32_t slot_id)
{
    if (item == NULL) return;
...
static void rs_init_items_range(lv_order_song_item_t *items, uint32_t total_items)
{
    uint32_t i;
...
bool lv_order_song_catalog_init(lv_order_song_catalog_t *catalog, uint32_t total_items)
{
    ...
    catalog->items = (lv_order_song_item_t *)OS_CALLOC(total_items, sizeof(lv_order_song_item_t), "catalog.items");
    ...
    if (pthread_mutex_init(&catalog->mutex, NULL) != 0) {
        OS_FLOW_LOG("catalog mutex init failed: total_items=%u", total_items);
        OS_FREE(catalog->items, "catalog.items");
        memset(catalog, 0, sizeof(*catalog));
        return false;
    }
...
{
    lv_order_song_item_t *new_items;

    if (catalog == NULL || total_items == 0U) return false;

    if (catalog->items != NULL && catalog->mutex_inited && catalog->capacity_items >= total_items) {
        pthread_mutex_lock(&catalog->mutex);
        memset(catalog->items, 0, (size_t)total_items * sizeof(lv_order_song_item_t));
        rs_init_items_range(catalog->items, total_items);
        catalog->total_items = total_items;
        pthread_mutex_unlock(&catalog->mutex);
        OS_HOT_LOG("catalog reset reuse: total=%u capacity=%u", total_items, catalog->capacity_items);
        return true;
    }

    new_items = (lv_order_song_item_t *)OS_CALLOC(total_items, sizeof(lv_order_song_item_t), "catalog.items.reset");
    if (new_items == NULL) {
        OS_FLOW_LOG("catalog reset alloc failed: total_items=%u", total_items);
        return false;
    }
    ...
    if (catalog->mutex_inited) {
        pthread_mutex_lock(&catalog->mutex);
        OS_FREE(catalog->items, "catalog.items.old");
        ...
        catalog->capacity_items = total_items;
        if (pthread_mutex_init(&catalog->mutex, NULL) != 0) {
            OS_FLOW_LOG("catalog reset mutex init failed: total_items=%u", total_items);
            OS_FREE(catalog->items, "catalog.items.reset");
            memset(catalog, 0, sizeof(*catalog));
            return false;
        }
        catalog->mutex_inited = 1U;
    ...
    OS_FLOW_LOG("catalog reset realloc: total=%u capacity=%u", total_items, catalog->capacity_items);
    return true;
}
...
{
    lv_order_song_catalog_t *c = (lv_order_song_catalog_t *)catalog;
...
bool lv_order_song_catalog_get_vlist_item(const lv_order_song_catalog_t *catalog, uint32_t index, lv_vlist_item_t *out)
{
    ...
    if (!lv_order_song_catalog_get_item(catalog, index, &item)) return false;
