#include "lv_favorite_song_catalog.h"
#include "../lv_favorite_song_debug.h"
#include <stdlib.h>
#include <string.h>

static void rs_init_item(lv_favorite_song_item_t *item, uint32_t slot_id)
{
    if (item == NULL) return;
...
static void rs_init_items_range(lv_favorite_song_item_t *items, uint32_t total_items)
{
    uint32_t i;
...
bool lv_favorite_song_catalog_init(lv_favorite_song_catalog_t *catalog, uint32_t total_items)
{
    if (catalog == NULL || total_items == 0U) return false;
    memset(catalog, 0, sizeof(*catalog));

    catalog->items = (lv_favorite_song_item_t *)RS_CALLOC(total_items, sizeof(lv_favorite_song_item_t), "catalog.items");
...
void lv_favorite_song_catalog_deinit(lv_favorite_song_catalog_t *catalog)
{
...
bool lv_favorite_song_catalog_reset(lv_favorite_song_catalog_t *catalog, uint32_t total_items)
{
    lv_favorite_song_item_t *new_items;

    if (catalog == NULL || total_items == 0U) return false;

    if (catalog->items != NULL && catalog->mutex_inited && catalog->capacity_items >= total_items) {
        pthread_mutex_lock(&catalog->mutex);
        memset(catalog->items, 0, (size_t)total_items * sizeof(lv_favorite_song_item_t));
        rs_init_items_range(catalog->items, total_items);
        catalog->total_items = total_items;
    ...
    new_items = (lv_favorite_song_item_t *)RS_CALLOC(total_items, sizeof(lv_favorite_song_item_t), "catalog.items.reset");
    if (new_items == NULL) {
...
uint32_t lv_favorite_song_catalog_count(const lv_favorite_song_catalog_t *catalog)
{
    return (catalog != NULL) ? catalog->total_items : 0U;
}

bool lv_favorite_song_catalog_get_item(const lv_favorite_song_catalog_t *catalog, uint32_t index, lv_favorite_song_item_t *out)
{
    lv_favorite_song_catalog_t *c = (lv_favorite_song_catalog_t *)catalog;
...
bool lv_favorite_song_catalog_set_item_by_slot(lv_favorite_song_catalog_t *catalog, uint32_t slot, const lv_favorite_song_item_t *item)
{
...
bool lv_favorite_song_catalog_truncate(lv_favorite_song_catalog_t *catalog, uint32_t new_total_items)
{
...
bool lv_favorite_song_catalog_get_vlist_item(const lv_favorite_song_catalog_t *catalog, uint32_t index, lv_vlist_item_t *out)
{
    lv_favorite_song_item_t item;
    if (out == NULL) return false;
    memset(out, 0, sizeof(*out));
    if (!lv_favorite_song_catalog_get_item(catalog, index, &item)) return false;
...
void lv_favorite_song_catalog_set_selected(lv_favorite_song_catalog_t *catalog, uint32_t index, bool selected)
{
