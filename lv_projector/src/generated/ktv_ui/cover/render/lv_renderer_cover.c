...
#define COVER_IMG_ZOOM_DEFAULT 256

static void cover_set_bubble_flags(lv_obj_t *obj)
{
    if (obj == NULL) return;
...
typedef struct {
    ...
    lv_obj_t *name_bar;
    lv_obj_t *lbl_name;
    char img_src[LV_COVER_PATH_MAX + 8];
} lv_renderer_cover_cell_t;

static const lv_cover_row_style_t *cover_style_of(lv_vlist_t *vlist)
{
    const lv_vlist_config_t *cfg = lv_vlist_get_config(vlist);
    return (const lv_cover_row_style_t *)cfg->renderer_style;
}
...
static void set_name_label(lv_obj_t *obj, const char *text, const lv_cover_row_style_t *s)
{
...
static uint16_t cover_calc_cover_zoom(const char *src, lv_coord_t dst_w, lv_coord_t dst_h)
{
...
static void cover_apply_cover_image(lv_renderer_cover_cell_t *cell, const char *src)
{
    lv_coord_t box_w;
    lv_coord_t box_h;
    uint16_t zoom;
    ...
    zoom = cover_calc_cover_zoom(src, box_w, box_h);
...
static void cover_clear_image(lv_renderer_cover_cell_t *cell)
{
    if (cell == NULL || cell->avatar_img == NULL) return;
    lv_img_set_src(cell->avatar_img, NULL);
    lv_img_set_zoom(cell->avatar_img, COVER_IMG_ZOOM_DEFAULT);
    lv_obj_center(cell->avatar_img);
    lv_obj_add_flag(cell->avatar_img, LV_OBJ_FLAG_HIDDEN);
    cell->img_src[0] = '\0';
}

static void cover_set_focus_style(lv_renderer_cover_cell_t *cell, bool focused)
{
    if (cell == NULL || cell->avatar_bg == NULL) return;

    if (focused) {
        lv_obj_set_style_border_color(cell->avatar_bg, lv_color_hex(0x00D7FF), 0);
        lv_obj_set_style_border_opa(cell->avatar_bg, LV_OPA_COVER, 0);
        lv_obj_set_style_border_width(cell->avatar_bg, 4, 0);
        lv_obj_set_style_outline_color(cell->avatar_bg, lv_color_hex(0x00D7FF), 0);
        lv_obj_set_style_outline_opa(cell->avatar_bg, LV_OPA_COVER, 0);
        lv_obj_set_style_outline_width(cell->avatar_bg, 2, 0);
        lv_obj_set_style_outline_pad(cell->avatar_bg, 2, 0);
        lv_obj_set_style_shadow_color(cell->avatar_bg, lv_color_hex(0x00D7FF), 0);
        lv_obj_set_style_shadow_opa(cell->avatar_bg, LV_OPA_70, 0);
        lv_obj_set_style_shadow_width(cell->avatar_bg, 18, 0);
        lv_obj_set_style_shadow_spread(cell->avatar_bg, 2, 0);
    } else {
        lv_obj_set_style_border_width(cell->avatar_bg, 0, 0);
        lv_obj_set_style_border_opa(cell->avatar_bg, LV_OPA_TRANSP, 0);
        lv_obj_set_style_outline_width(cell->avatar_bg, 0, 0);
        lv_obj_set_style_outline_opa(cell->avatar_bg, LV_OPA_TRANSP, 0);
        lv_obj_set_style_outline_pad(cell->avatar_bg, 0, 0);
        lv_obj_set_style_shadow_opa(cell->avatar_bg, LV_OPA_TRANSP, 0);
        lv_obj_set_style_shadow_width(cell->avatar_bg, 0, 0);
        lv_obj_set_style_shadow_spread(cell->avatar_bg, 0, 0);
    }
}

static void *cover_create_cell(lv_vlist_t *vlist, lv_obj_t *parent)
{
    lv_renderer_cover_cell_t *cell;
    const lv_cover_row_style_t *s = cover_style_of(vlist);

    if (s == NULL) return NULL;

    cell = (lv_renderer_cover_cell_t *)calloc(1, sizeof(*cell));
    ...
    cover_set_bubble_flags(cell->root);
    ...
    lv_obj_set_style_border_width(cell->avatar_bg, 0, 0);
    cover_set_bubble_flags(cell->avatar_bg);

    cell->avatar_img = lv_img_create(cell->avatar_bg);
    lv_img_set_zoom(cell->avatar_img, COVER_IMG_ZOOM_DEFAULT);
    lv_obj_center(cell->avatar_img);
    cover_set_bubble_flags(cell->avatar_img);
    ...
    if (s->name_h > 0) {
        ...
        /* 默认隐藏，bind_cell 时按 ready/cover_ready 决定是否露出 */
        lv_obj_add_flag(cell->name_bar, LV_OBJ_FLAG_HIDDEN);
        cover_set_bubble_flags(cell->name_bar);

        cell->lbl_name = lv_label_create(cell->name_bar);
        lv_obj_set_width(cell->lbl_name, s->name_w);
        lv_label_set_long_mode(cell->lbl_name, LV_LABEL_LONG_WRAP);
        lv_obj_set_size(cell->lbl_name, s->name_w, s->name_h);
        ...
        cover_set_bubble_flags(cell->lbl_name);
    } else {
        cell->lbl_name = make_label(cell->root, s->name_x, s->name_y, s->name_w);
        cover_set_bubble_flags(cell->lbl_name);
    }
    cover_clear_image(cell);
    return cell;
}

static void cover_destroy_cell(lv_vlist_t *vlist, void *renderer_ctx)
{
    lv_renderer_cover_cell_t *cell = (lv_renderer_cover_cell_t *)renderer_ctx;
    (void)vlist;
    if (cell != NULL) {
        cover_clear_image(cell);
        free(cell);
...
static lv_obj_t *cover_get_root(lv_vlist_t *vlist, void *renderer_ctx)
{
    lv_renderer_cover_cell_t *cell = (lv_renderer_cover_cell_t *)renderer_ctx;
    (void)vlist;
...
static void cover_bind_cell(lv_vlist_t *vlist, void *renderer_ctx, const lv_vlist_item_t *item)
{
    lv_renderer_cover_cell_t *cell = (lv_renderer_cover_cell_t *)renderer_ctx;
    const lv_vlist_config_t *cfg = lv_vlist_get_config(vlist);
    const lv_cover_row_style_t *s = cover_style_of(vlist);
    ...
    adapter = (lv_cover_adapter_t *)cfg->user_ctx;
    if (adapter == NULL) return;

    if (!lv_cover_adapter_get_business_item(adapter, item->item_id, &biz)) return;

    if (biz.selected) {
        lv_obj_set_style_bg_color(cell->root, s->checked_bg_color, 0);
        lv_obj_set_style_bg_opa(cell->root, s->checked_bg_opa, 0);
        cover_set_focus_style(cell, true);
    } else {
        lv_obj_set_style_bg_color(cell->root, s->bg_color, 0);
        lv_obj_set_style_bg_opa(cell->root, s->bg_opa, 0);
        cover_set_focus_style(cell, false);
    }
    ...
    if (biz.cover_ready && biz.cover_local_path[0] != '\0') {
        char next_src[sizeof(cell->img_src)];
        build_lvgl_fs_path(biz.cover_local_path, next_src, sizeof(next_src));
        if (strcmp(cell->img_src, next_src) != 0) {
            cover_clear_image(cell);
            snprintf(cell->img_src, sizeof(cell->img_src), "%s", next_src);
            cover_apply_cover_image(cell, cell->img_src);
        } else {
            cover_apply_cover_image(cell, cell->img_src);
        }
    } else {
...
static void cover_set_pos(lv_vlist_t *vlist, void *renderer_ctx, lv_coord_t x, lv_coord_t y)
{
    lv_renderer_cover_cell_t *cell = (lv_renderer_cover_cell_t *)renderer_ctx;
    (void)vlist;
...
const lv_vlist_renderer_ops_t g_lv_renderer_cover_ops = {
    .create_cell = cover_create_cell,
    .destroy_cell = cover_destroy_cell,
    .get_root = cover_get_root,
    .bind_cell = cover_bind_cell,
    .set_pos = cover_set_pos
};
