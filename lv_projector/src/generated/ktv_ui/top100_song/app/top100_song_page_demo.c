...
#include <string.h>
static top100_song_page_demo_t g_page1;
                             ...
                             const lv_top100_song_view_style_t *view_style)
{
    ...
    if (lv_top100_song_adapter_start(&page->adapter, view_style, total_count, batch_size) != 0) {
        lv_obj_del(page->root);
        page->root = NULL;
        return -3;
    }

    page->vlist = lv_top100_song_adapter_create_vlist(&page->adapter, page->root);
    if (page->vlist == NULL) {
        lv_top100_song_adapter_stop(&page->adapter);
        lv_obj_del(page->root);
        page->root = NULL;
        return -4;
    }
...
{
    lv_top100_song_adapter_stop(&g_page1.adapter);
...
void top100_song_page_demo_reset(uint32_t total_count)
{
    ...
    lv_top100_song_adapter_reset(&g_page1.adapter, total_count);
}
...
static void app_ui_init(void)
{
    ...
    if(initial_flag == 0)
    {
        lv_top100_song_default_style_init();
        lv_top100_song_style_3x2_init();
        lv_top100_song_style_2x3_init();
        lv_top100_song_style_1x8_init();
        initial_flag ++;
    }
}
...
void demo_app_top100_songs_list(lv_obj_t *parent)
{
    ...
    top100_song_page_demo_open(&g_page1,
                            parent,
                            100,
                            50,
                            &g_lv_top100_song_default_style);
}