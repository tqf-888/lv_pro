#ifndef LV_TOP100_SONG_CATALOG_H
#define LV_TOP100_SONG_CATALOG_H
#ifdef __cplusplus
extern "C" {
#endif
#include <stdbool.h>
...
#define LV_TOP100_SONG_NAME_MAX 64
typedef struct {
    uint32_t slot_id;
    uint32_t song_id;
    uint8_t ready;
    ...
    uint8_t selected;
    char index_text[16];
    char name[LV_TOP100_SONG_NAME_MAX];
    char artist[LV_TOP100_SONG_NAME_MAX];
    char text_a[32];
    char text_b[32];
    char text_c[32];
    char text_d[32];
...
} lv_top100_song_item_t;
typedef struct {
    uint32_t total_items;
    uint32_t capacity_items;
    lv_top100_song_item_t *items;
    pthread_mutex_t mutex;
    uint8_t mutex_inited;
} lv_top100_song_catalog_t;
