...
static lv_top100_song_adapter_t *g_top100_song_default = NULL;
 ...
 * 生成批次文件路径。
 * 新逻辑优先使用带 epoch 的文件名：
 *   /tmp/top100_song_batch_<epoch>_<batch>.json
 * 这样旧请求和新请求不会写到同一个文件里。
 *
 * 下面保留旧路径兜底：
 *   /tmp/top100_song_batch_<batch>.json
 */
static int rs_batch_build_path(uint32_t batch_index, uint32_t request_epoch, char *buf, size_t size)
{
    ...
    if (request_epoch != 0U) {
        n = snprintf(buf, size, "/tmp/top100_song_batch_%u_%u.json", request_epoch, batch_index);
        if (n >= 0 && (size_t)n < size) {
            return 0;
        }
    }

    n = snprintf(buf, size, "/tmp/top100_song_batch_%u.json", batch_index);
...
/* 给歌曲行填默认文字样式 */
static void rs_fill_default_text_style(lv_top100_song_item_t *it)
{
    if (it == NULL) return;
 ...
 */
static int rs_sync_batch_from_json(lv_top100_song_adapter_t *adapter,
                                   ...
                                   rs_batch_sync_meta_t *meta)
{
    ...
    if (rs_batch_build_path(batch_index, request_epoch, path, sizeof(path)) != 0) return -1;
    if (rs_read_text_file(path, &json_str, &file_len) != 0) return -1;
    ...
    data = cJSON_GetObjectItem(result, "hot_songs");
    ...
    for (i = 0; i < array_size && (start_id + (uint32_t)i) < end_id; ++i) {
        ...
        cJSON *is_vip = cJSON_GetObjectItem(node, "is_vip");
        cJSON *mv_id = cJSON_GetObjectItem(node, "mv_id");
        ...
        (void)lv_top100_song_catalog_set_item_by_slot(&adapter->catalog, item.slot_id, &item);
    }
...
{
    lv_top100_song_adapter_t *adapter = (lv_top100_song_adapter_t *)user_ctx;
    return (adapter != NULL) ? lv_top100_song_catalog_count(&adapter->catalog) : 0U;
}
 ...
 * - 正在 loading 的不再重复请求
 */
static void rs_request_batch(lv_top100_song_adapter_t *adapter, uint32_t batch_index)
{
    ...
    if (adapter->end_reached && batch_index > adapter->last_batch_index) {
        printf("[top100_song] 跳过请求: batch=%u 已超过最后一页 batch=%u\n",
               batch_index,
    ...
    fetch_top100_song_batch((int)batch_index);
}

static int rs_finish_first_batch_gate_if_needed(lv_top100_song_adapter_t *adapter,
                                                uint32_t batch_index)
{
    ...
    printf("[top100_song] 首批回调完成，补做最后一次reset: total=%u epoch=%u\n",
           deferred_total,
           adapter->request_epoch);

    lv_top100_song_adapter_reset(adapter, deferred_total);
    return 1;
}
...
static bool rs_get_item(uint32_t index, lv_vlist_item_t *out, void *user_ctx)
{
    ...
    return lv_top100_song_catalog_get_vlist_item(&adapter->catalog, index, out);
}

/* 根据点击横坐标判断点的是哪一列 */
static const char *rs_hit_area_name(const lv_top100_song_row_style_t *s, lv_coord_t rel_x)
{
    if (s == NULL) return "unknown";
    if (rel_x >= s->idx_x    && rel_x < (s->idx_x + s->idx_w))       return "idx";
    if (rel_x >= s->name_x   && rel_x < (s->name_x + s->name_w))     return "name";
    if (rel_x >= s->artist_x && rel_x < (s->artist_x + s->artist_w)) return "artist";
    if (rel_x >= s->a_x      && rel_x < (s->a_x + s->a_w))           return "a";
    if (rel_x >= s->b_x      && rel_x < (s->b_x + s->b_w))           return "b";
    if (rel_x >= s->c_x      && rel_x < (s->c_x + s->c_w))           return "c";
    if (rel_x >= s->d_x      && rel_x < (s->d_x + s->d_w))           return "d";
    if (rel_x >= s->e_x      && rel_x < (s->e_x + s->e_w))           return "e";
    if (rel_x >= s->f_x      && rel_x < (s->f_x + s->f_w))           return "f";
    return "unknown";
}
                             ...
                             lv_coord_t rel_y)
{
    ...
    if (!lv_top100_song_catalog_get_item(&adapter->catalog, item_id, &item)) {
        return;
    }
    lv_top100_song_row_style_t  *s = &adapter->view_style->row_style;
    ...
    if (item_changed != 0)
    {
        (void)lv_top100_song_catalog_set_item_by_slot(&adapter->catalog, item_id, &item);
...
static int rs_pop_one_batch(lv_top100_song_adapter_t *adapter,
                            uint32_t *batch_index,
                            uint8_t *success,
                            uint32_t *request_epoch)
{
...
static void rs_apply_last_page_if_needed(lv_top100_song_adapter_t *adapter,
                                        uint32_t batch_index,
                                        const rs_batch_sync_meta_t *meta)
{
    ...
    if (final_total == adapter->total_count) {
        printf("[top100_song] 已到最后一页: batch=%u, recv=%u, batch_size=%u, final_total=%u\n",
               batch_index,
               meta->data_count,
               adapter->batch_size,
               final_total);
     ...
     */
    (void)lv_top100_song_catalog_truncate(&adapter->catalog, final_total);

    printf("[top100_song] 已到最后一页: batch=%u, recv=%u, batch_size=%u, final_total=%u, json_total_count=%u%s\n",
           batch_index,
           meta->data_count,
           adapter->batch_size,
           final_total,
...
{
    lv_top100_song_adapter_t *adapter = (lv_top100_song_adapter_t *)timer->user_data;
    uint32_t batch_index;
    uint32_t request_epoch;
    uint8_t success;
    ...
    while (rs_pop_one_batch(adapter, &batch_index, &success, &request_epoch)) {
        ...
        if (!success) continue;
        if (rs_sync_batch_from_json(adapter, batch_index, request_epoch, &meta) != 0) continue;
...
int lv_top100_song_adapter_start(lv_top100_song_adapter_t *adapter,
                               ...
                               uint32_t batch_size)
{
    ...
    if (!lv_top100_song_catalog_init(&adapter->catalog, total_count)) {
        return -1;
    }
    ...
    if (adapter->batch_ready == NULL ||
        ...
        lv_top100_song_adapter_stop(adapter);
        return -1;
    }
    ...
    adapter->deferred_reset = 0U;
    adapter->deferred_total_count = 0U;
    g_top100_song_default = adapter;
    return 0;
}

/* 释放 adapter 内部资源 */
void lv_top100_song_adapter_stop(lv_top100_song_adapter_t *adapter)
{
    if (adapter == NULL) {
        return;
    }
    ...
    lv_top100_song_catalog_deinit(&adapter->catalog);

    if (g_top100_song_default == adapter) {
        g_top100_song_default = NULL;
    }
...
/* 创建并绑定虚拟列表 */
lv_vlist_t *lv_top100_song_adapter_create_vlist(lv_top100_song_adapter_t *adapter, lv_obj_t *parent)
{
    ...
    cfg.renderer_ops = &g_lv_renderer_top100_song_ops;
 ...
 * 4. 立即主动请求第 0 批
 */
void lv_top100_song_adapter_reset(lv_top100_song_adapter_t *adapter, uint32_t total_count)
{
    ...
    (void)lv_top100_song_catalog_reset(&adapter->catalog, total_count);
    ...
    if (adapter->first_batch_inflight) {
        ...
        printf("[top100_song] reset合并: 首批仍在飞，延后到首批回调后再做 total=%u epoch=%u\n",
               total_count,
               adapter->request_epoch);
        return;
    }
...
/* 业务层读取某一行完整数据 */
bool lv_top100_song_adapter_get_business_item(lv_top100_song_adapter_t *adapter, uint32_t item_id, lv_top100_song_item_t *out)
{
    if (adapter == NULL) {
        return false;
    }
    return lv_top100_song_catalog_get_item(&adapter->catalog, item_id, out);
}

/* 切换选中状态并刷新单行 */
void lv_top100_song_adapter_toggle_selected(lv_top100_song_adapter_t *adapter, uint32_t item_id)
{
    lv_top100_song_item_t item;
    ...
    lv_top100_song_catalog_set_selected(&adapter->catalog, item_id, !item.selected);
    if (adapter->vlist != NULL) {
        lv_vlist_notify_item_changed(adapter->vlist, item_id);
    }
}
...
void lv_top100_song_adapter_notify_batch_ready(lv_top100_song_adapter_t *adapter, uint32_t batch_index, bool success)
{
    if (adapter == NULL) {
        return;
    }

    lv_top100_song_adapter_notify_batch_ready_with_epoch(adapter,
                                                       batch_index,
                                                       success,
                                                       adapter->request_epoch);
}
...
void lv_top100_song_adapter_notify_batch_ready_with_epoch(lv_top100_song_adapter_t *adapter,
                                                        uint32_t batch_index,
                                                        bool success,
                                                        uint32_t request_epoch)
{
...
/* 取默认 adapter */
lv_top100_song_adapter_t *lv_top100_song_adapter_get_default(void)
{
    return g_top100_song_default;
}

/* 读取当前代号 */
uint32_t lv_top100_song_adapter_get_request_epoch(const lv_top100_song_adapter_t *adapter)
{
    return (adapter != NULL) ? adapter->request_epoch : 0U;
}

/* 默认 adapter 的当前代号 */
uint32_t lv_top100_song_adapter_get_default_request_epoch(void)
{
    return lv_top100_song_adapter_get_request_epoch(g_top100_song_default);
}

/* 默认 adapter 的旧接口快捷方式 */
void lv_top100_song_adapter_notify_default_batch_ready(uint32_t batch_index, bool success)
{
    lv_top100_song_adapter_notify_default_batch_ready_with_epoch(batch_index,
                                                               success,
                                                               lv_top100_song_adapter_get_default_request_epoch());
}

/* 默认 adapter 的带 epoch 快捷方式 */
void lv_top100_song_adapter_notify_default_batch_ready_with_epoch(uint32_t batch_index,
                                                                bool success,
                                                                uint32_t request_epoch)
{
    lv_top100_song_adapter_notify_batch_ready_with_epoch(g_top100_song_default,
                                                       batch_index,
                                                       success,
                                                       request_epoch);
}
