...
typedef struct {
    ...
    lv_obj_t *lbl_e;
    lv_obj_t *lbl_f;
} lv_renderer_top100_song_cell_t;

static const lv_top100_song_row_style_t *rs_style_of(lv_vlist_t *vlist)
{
    const lv_vlist_config_t *cfg = lv_vlist_get_config(vlist);
    return (const lv_top100_song_row_style_t *)cfg->renderer_style;
}
...
static void rs_set_label(lv_obj_t *obj,const char *text,uint32_t color_hex,lv_opa_t opa,const lv_font_t *font,const lv_top100_song_row_style_t *s)
{
...
{
    lv_renderer_top100_song_cell_t *cell;
    const lv_top100_song_row_style_t *s = rs_style_of(vlist);
    if (s == NULL) return NULL;
    cell = (lv_renderer_top100_song_cell_t *)RS_CALLOC(1, sizeof(*cell), "renderer.cell");
...
static lv_obj_t *rs_get_root(lv_vlist_t *vlist, void *renderer_ctx) { (void)vlist; return renderer_ctx ? ((lv_renderer_top100_song_cell_t *)renderer_ctx)->root : NULL; }

static void rs_bind_cell(lv_vlist_t *vlist, void *renderer_ctx, const lv_vlist_item_t *item)
{
    lv_renderer_top100_song_cell_t *cell = (lv_renderer_top100_song_cell_t *)renderer_ctx;
    const lv_vlist_config_t *cfg = lv_vlist_get_config(vlist);
    const lv_top100_song_row_style_t *s = rs_style_of(vlist);
    lv_top100_song_adapter_t *adapter;
    lv_top100_song_item_t biz;

    if (cell == NULL || item == NULL || cfg == NULL || s == NULL) return;
    adapter = (lv_top100_song_adapter_t *)cfg->user_ctx;
    if (adapter == NULL) return;
    if (!lv_top100_song_adapter_get_business_item(adapter, item->item_id, &biz)) return;
...
const lv_vlist_renderer_ops_t g_lv_renderer_top100_song_ops = {
