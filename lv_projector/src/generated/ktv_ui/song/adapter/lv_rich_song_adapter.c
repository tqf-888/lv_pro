...
static lv_rich_song_adapter_t *g_rich_song_default = NULL;
static uint32_t g_rich_song_epoch_seed = 0U;

static uint32_t rs_next_request_epoch(void)
{
    g_rich_song_epoch_seed += 1U;
    if (g_rich_song_epoch_seed == 0U) {
        g_rich_song_epoch_seed = 1U;
    }
    return g_rich_song_epoch_seed;
}
 ...
 * 生成批次文件路径。
 * 新逻辑优先使用带 epoch 的文件名：
 *   /tmp/song_batch_<epoch>_<batch>.json
 * 这样旧请求和新请求不会写到同一个文件里。
 *
 * 下面保留旧路径兜底：
 *   /tmp/song_batch_<batch>.json
 */
static int rs_batch_build_path(uint32_t batch_index, uint32_t request_epoch, char *buf, size_t size)
{
    ...
    if (request_epoch != 0U) {
        n = snprintf(buf, size, "/tmp/song_batch_%u_%u.json", request_epoch, batch_index);
        if (n >= 0 && (size_t)n < size) {
            return 0;
        }
    }

    n = snprintf(buf, size, "/tmp/song_batch_%u.json", batch_index);
...
/* 给歌曲行填默认文字样式 */
static void rs_fill_default_text_style(lv_rich_song_item_t *it)
{
    if (it == NULL) return;
 ...
 */
static int rs_sync_batch_from_json(lv_rich_song_adapter_t *adapter,
...
{
    lv_rich_song_adapter_t *adapter = (lv_rich_song_adapter_t *)user_ctx;
 ...
 * - 正在 loading 的不再重复请求
 */
static void rs_request_batch(lv_rich_song_adapter_t *adapter, uint32_t batch_index)
{
    ...
    if (adapter->end_reached && batch_index > adapter->last_batch_index) {
        printf("[rich_song] 跳过请求: batch=%u 已超过最后一页 batch=%u\n",
               batch_index,
    ...
    fetch_song_batch((int)batch_index);
}

static int rs_finish_first_batch_gate_if_needed(lv_rich_song_adapter_t *adapter,
                                                uint32_t batch_index)
{
    ...
    printf("[rich_song] 首批回调完成，补做最后一次reset: total=%u epoch=%u\n",
           deferred_total,
           adapter->request_epoch);

    lv_rich_song_adapter_reset(adapter, deferred_total);
    return 1;
}
...
static int rs_pop_one_batch(lv_rich_song_adapter_t *adapter,
                            uint32_t *batch_index,
                            uint8_t *success,
                            uint32_t *request_epoch)
{
...
static void rs_apply_last_page_if_needed(lv_rich_song_adapter_t *adapter,
                                        uint32_t batch_index,
                                        const rs_batch_sync_meta_t *meta)
{
    ...
    if (final_total == adapter->total_count) {
        printf("[rich_song] 已到最后一页: batch=%u, recv=%u, batch_size=%u, final_total=%u\n",
               batch_index,
               meta->data_count,
               adapter->batch_size,
               final_total);
    ...
    printf("[rich_song] 已到最后一页: batch=%u, recv=%u, batch_size=%u, final_total=%u, json_total_count=%u%s\n",
           batch_index,
           meta->data_count,
           adapter->batch_size,
           final_total,
...
{
    lv_rich_song_adapter_t *adapter = (lv_rich_song_adapter_t *)timer->user_data;
    uint32_t batch_index;
    uint32_t request_epoch;
    uint8_t success;
...
int lv_rich_song_adapter_start(lv_rich_song_adapter_t *adapter,
                               ...
                               uint32_t batch_size)
{
    ...
    if (adapter->batch_ready == NULL ||
        ...
        lv_rich_song_adapter_stop(adapter);
        return -1;
    }
    ...
    adapter->started = 1U;
    adapter->request_epoch = rs_next_request_epoch();
    adapter->end_reached = 0U;
    adapter->last_batch_index = (adapter->batch_count > 0U) ? (adapter->batch_count - 1U) : 0U;
    adapter->first_batch_inflight = 0U;
    adapter->deferred_reset = 0U;
    adapter->deferred_total_count = 0U;
    g_rich_song_default = adapter;
    return 0;
}

/* 释放 adapter 内部资源 */
void lv_rich_song_adapter_stop(lv_rich_song_adapter_t *adapter)
{
    if (adapter == NULL) {
        return;
    }
    ...
    if (g_rich_song_default == adapter) {
        g_rich_song_default = NULL;
    }
...
/* 创建并绑定虚拟列表 */
lv_vlist_t *lv_rich_song_adapter_create_vlist(lv_rich_song_adapter_t *adapter, lv_obj_t *parent)
{
 ...
 * 4. 立即主动请求第 0 批
 */
void lv_rich_song_adapter_reset(lv_rich_song_adapter_t *adapter, uint32_t total_count)
{
    ...
    if (adapter->first_batch_inflight) {
        ...
        printf("[rich_song] reset合并: 首批仍在飞，延后到首批回调后再做 total=%u epoch=%u\n",
               total_count,
               adapter->request_epoch);
        return;
    }
...
/* 业务层读取某一行完整数据 */
bool lv_rich_song_adapter_get_business_item(lv_rich_song_adapter_t *adapter, uint32_t item_id, lv_rich_song_item_t *out)
{
    if (adapter == NULL) {
        return false;
    }
...
/* 切换选中状态并刷新单行 */
void lv_rich_song_adapter_toggle_selected(lv_rich_song_adapter_t *adapter, uint32_t item_id)
{
    lv_rich_song_item_t item;
...
void lv_rich_song_adapter_notify_batch_ready(lv_rich_song_adapter_t *adapter, uint32_t batch_index, bool success)
{
    if (adapter == NULL) {
        return;
    }

    lv_rich_song_adapter_notify_batch_ready_with_epoch(adapter,
                                                       batch_index,
                                                       success,
                                                       adapter->request_epoch);
}
...
void lv_rich_song_adapter_notify_batch_ready_with_epoch(lv_rich_song_adapter_t *adapter,
                                                        uint32_t batch_index,
                                                        bool success,
                                                        uint32_t request_epoch)
{
...
/* 取默认 adapter */
lv_rich_song_adapter_t *lv_rich_song_adapter_get_default(void)
{
    return g_rich_song_default;
}

/* 读取当前代号 */
uint32_t lv_rich_song_adapter_get_request_epoch(const lv_rich_song_adapter_t *adapter)
{
    return (adapter != NULL) ? adapter->request_epoch : 0U;
}

/* 默认 adapter 的当前代号 */
uint32_t lv_rich_song_adapter_get_default_request_epoch(void)
{
    return lv_rich_song_adapter_get_request_epoch(g_rich_song_default);
}

/* 默认 adapter 的旧接口快捷方式 */
void lv_rich_song_adapter_notify_default_batch_ready(uint32_t batch_index, bool success)
{
    lv_rich_song_adapter_notify_default_batch_ready_with_epoch(batch_index,
                                                               success,
                                                               lv_rich_song_adapter_get_default_request_epoch());
}

/* 默认 adapter 的带 epoch 快捷方式 */
void lv_rich_song_adapter_notify_default_batch_ready_with_epoch(uint32_t batch_index,
                                                                bool success,
                                                                uint32_t request_epoch)
{
    lv_rich_song_adapter_notify_batch_ready_with_epoch(g_rich_song_default,
                                                       batch_index,
                                                       success,
                                                       request_epoch);
}
