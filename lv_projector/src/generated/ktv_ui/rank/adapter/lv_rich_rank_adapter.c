#include "lv_rich_rank_adapter.h"
#include "lv_renderer_rich_song.h"
#include "cJSON.h"
#include "ktv.h"

#include <stdlib.h>
#include <string.h>
#include <stdio.h>

#include "db_list_pro_worker.h"
#include "ktv_player_ui.h"
#include "ktv_cloud_order_api.h"
#include "db_list_pro_thread.h"
#include "gui_guider.h"

LV_FONT_DECLARE(lv_font_Regular_20);
LV_FONT_DECLARE(lv_font_ktv_30);

static lv_rich_rank_adapter_t *g_rich_rank_default = NULL;

typedef struct {
    uint32_t data_count;
    uint32_t json_total_count;
    uint8_t json_total_count_valid;
    uint8_t reached_end;
    uint32_t final_total_count;
} rr_batch_sync_meta_t;

static uint32_t rr_safe_positive_u32(uint32_t v, uint32_t defv)
{
    return (v == 0U) ? defv : v;
}

static int rr_batch_build_path(uint32_t batch_index, uint32_t request_epoch, char *buf, size_t size)
{
    int n;

    if (buf == NULL || size == 0U) return -1;

    if (request_epoch != 0U) {
        n = snprintf(buf, size, "/tmp/4_15_song_%u_%u.json", request_epoch, batch_index);
        if (n >= 0 && (size_t)n < size) {
            return 0;
        }
    }

    n = snprintf(buf, size, "/tmp/4_15_song_%u.json", batch_index);
    if (n < 0 || (size_t)n >= size) return -1;
    return 0;
}

static int rr_read_text_file(const char *path, char **out_buf, long *out_len)
{
    FILE *fp;
    long len;
    char *buf;

    if (out_buf == NULL || out_len == NULL || path == NULL) return -1;
    *out_buf = NULL;
    *out_len = 0;

    fp = fopen(path, "rb");
    if (fp == NULL) return -1;

    if (fseek(fp, 0, SEEK_END) != 0) {
        fclose(fp);
        return -1;
    }
    len = ftell(fp);
    if (len <= 0) {
        fclose(fp);
        return -1;
    }
    if (fseek(fp, 0, SEEK_SET) != 0) {
        fclose(fp);
        return -1;
    }

    buf = (char *)malloc((size_t)len + 1U);
    if (buf == NULL) {
        fclose(fp);
        return -1;
    }

    if (fread(buf, 1, (size_t)len, fp) != (size_t)len) {
        free(buf);
        fclose(fp);
        return -1;
    }

    fclose(fp);
    buf[len] = '\0';

    *out_buf = buf;
    *out_len = len;
    return 0;
}

static void rr_fill_default_text_style(lv_rich_song_item_t *it)
{
    if (it == NULL) return;

    it->color_idx = 0xFFFFFF;
    it->opa_idx = 255;
    it->font_idx = &lv_font_Regular_20;

    it->color_name = 0xFFFFFF;
    it->opa_name = 255;
    it->font_name = &lv_font_Regular_20;

    it->color_artist = 0xBBBBBB;
    it->opa_artist = 255;
    it->font_artist = &lv_font_Regular_20;

    it->color_a = 0xFFFF00;
    it->opa_a = 255;
    it->font_a = &lv_font_ktv_30;

    it->color_b = 0xFFFF00;
    it->opa_b = 255;
    it->font_b = &lv_font_ktv_30;

    it->color_c = 0xAAAAAA;
    it->opa_c = 255;
    it->font_c = &lv_font_ktv_30;

    it->color_d = 0x66CCFF;
    it->opa_d = 220;
    it->font_d = &lv_font_ktv_30;

    it->color_e = 0xFF99CC;
    it->opa_e = 200;
    it->font_e = &lv_font_ktv_30;

    it->color_f = 0x99FF99;
    it->opa_f = 180;
    it->font_f = &lv_font_ktv_30;
}

static cJSON *rr_get_first_object_item(cJSON *obj, const char *k1, const char *k2, const char *k3)
{
    cJSON *node = NULL;

    if (obj == NULL) return NULL;

    if (k1 != NULL) {
        node = cJSON_GetObjectItem(obj, k1);
        if (node != NULL) return node;
    }
    if (k2 != NULL) {
        node = cJSON_GetObjectItem(obj, k2);
        if (node != NULL) return node;
    }
    if (k3 != NULL) {
        node = cJSON_GetObjectItem(obj, k3);
        if (node != NULL) return node;
    }

    return NULL;
}

static const char *rr_get_string_value(cJSON *obj, const char *k1, const char *k2, const char *k3)
{
    cJSON *node = rr_get_first_object_item(obj, k1, k2, k3);
    if (node != NULL && cJSON_IsString(node) && node->valuestring != NULL) {
        return node->valuestring;
    }
    return NULL;
}

static int rr_get_int_value(cJSON *obj, const char *k1, const char *k2, const char *k3, int defv)
{
    cJSON *node = rr_get_first_object_item(obj, k1, k2, k3);
    if (node != NULL && cJSON_IsNumber(node)) {
        return node->valueint;
    }
    return defv;
}

/*
 * 排行榜 json 兼容两种常见结构：
 * 1. result.songs   <- 你这次给的样例就是这个
 * 2. result.data    <- 复用 song 业务时常见的结构
 */
static cJSON *rr_get_result_array(cJSON *result)
{
    cJSON *arr;

    if (result == NULL) return NULL;

    arr = cJSON_GetObjectItem(result, "songs");
    if (cJSON_IsArray(arr)) return arr;

    arr = cJSON_GetObjectItem(result, "data");
    if (cJSON_IsArray(arr)) return arr;

    return NULL;
}

static void rr_apply_total_count_hint_if_needed(lv_rich_rank_adapter_t *adapter,
                                                const rr_batch_sync_meta_t *meta)
{
    uint32_t final_total;
    uint32_t final_batch_count;

    if (adapter == NULL || meta == NULL || !meta->json_total_count_valid) {
        return;
    }

    final_total = meta->json_total_count;
    if (final_total > adapter->total_count) {
        return;
    }

    if (final_total == adapter->total_count) {
        return;
    }

    adapter->total_count = final_total;
    final_batch_count = (final_total == 0U)
                      ? 0U
                      : ((final_total + adapter->batch_size - 1U) / adapter->batch_size);
    adapter->batch_count = final_batch_count;
    adapter->last_batch_index = (final_batch_count > 0U) ? (final_batch_count - 1U) : 0U;

    (void)lv_rich_song_catalog_truncate(&adapter->catalog, final_total);

    printf("[rich_rank] 根据 json_total_count 收缩总数: total=%u, batch_count=%u\n",
           final_total,
           final_batch_count);

    if (adapter->vlist != NULL) {
        lv_vlist_reload(adapter->vlist);
    }
}

static int rr_sync_batch_from_json(lv_rich_rank_adapter_t *adapter,
                                   uint32_t batch_index,
                                   uint32_t request_epoch,
                                   rr_batch_sync_meta_t *meta)
{
    char path[128];
    char *json_str = NULL;
    long file_len = 0;
    cJSON *root = NULL;
    cJSON *result = NULL;
    cJSON *data = NULL;
    cJSON *json_total_count = NULL;
    uint32_t start_id, end_id;
    int array_size, i;

    if (meta != NULL) {
        memset(meta, 0, sizeof(*meta));
    }

    if (adapter == NULL) return -1;
    if (rr_batch_build_path(batch_index, request_epoch, path, sizeof(path)) != 0) return -1;
    if (rr_read_text_file(path, &json_str, &file_len) != 0) return -1;

    root = cJSON_Parse(json_str);
    free(json_str);
    if (root == NULL) return -1;

    result = cJSON_GetObjectItem(root, "result");
    if (result == NULL) {
        cJSON_Delete(root);
        return -1;
    }

    data = rr_get_result_array(result);
    if (!cJSON_IsArray(data)) {
        cJSON_Delete(root);
        return -1;
    }

    json_total_count = cJSON_GetObjectItem(result, "total_count");
    if (meta != NULL && json_total_count != NULL && cJSON_IsNumber(json_total_count) && json_total_count->valueint >= 0) {
        meta->json_total_count = (uint32_t)json_total_count->valueint;
        meta->json_total_count_valid = 1U;
    }

    start_id = batch_index * adapter->batch_size;
    end_id = start_id + adapter->batch_size;
    if (end_id > adapter->total_count) end_id = adapter->total_count;

    array_size = cJSON_GetArraySize(data);
    if (meta != NULL) {
        meta->data_count = (array_size > 0) ? (uint32_t)array_size : 0U;
    }

    for (i = 0; i < array_size && (start_id + (uint32_t)i) < end_id; ++i) {
        cJSON *node = cJSON_GetArrayItem(data, i);
        const char *name;
        const char *artist;
        const char *rank_no;
        int vip_flag;
        int mv_flag;
        int mp3_flag;
        int melp_flag;
        int hd_mv_flag;
        int preview_flag;
        lv_rich_song_item_t item;

        memset(&item, 0, sizeof(item));
        rr_fill_default_text_style(&item);
        item.slot_id = start_id + (uint32_t)i;
        item.song_id = (uint32_t)rr_get_int_value(node, "songid", "id", "music_id", (int)item.slot_id);
        item.ready = 1U;
        item.loading = 0U;
        item.selected = 0U;

        rank_no = rr_get_string_value(node, "number", "rank", "ranking");
        if (rank_no != NULL && rank_no[0] != '\0') {
            snprintf(item.index_text, sizeof(item.index_text), "%s", rank_no);
        } else {
            snprintf(item.index_text, sizeof(item.index_text), "%u", item.slot_id + 1U);
        }

        name = rr_get_string_value(node, "songname", "name", "title");
        if (name != NULL) {
            snprintf(item.name, sizeof(item.name), "%s", name);
        } else {
            snprintf(item.name, sizeof(item.name), "未知");
        }

        artist = rr_get_string_value(node, "artist", "singer", "artist_name");
        if (artist != NULL) {
            snprintf(item.artist, sizeof(item.artist), "%s", artist);
        } else {
            item.artist[0] = '\0';
        }

        vip_flag     = rr_get_int_value(node, "is_vip", NULL, NULL, 0);
        mv_flag      = rr_get_int_value(node, "mv_id", NULL, NULL, 0);
        mp3_flag     = rr_get_int_value(node, "is_mp3", NULL, NULL, 0);
        melp_flag    = rr_get_int_value(node, "is_melp", NULL, NULL, 0);
        hd_mv_flag   = rr_get_int_value(node, "is_hd_mv", NULL, NULL, 0);
        preview_flag = rr_get_int_value(node, "is_mp3_preview", NULL, NULL, 0);

        item.text_a[0] = '\0';
        item.text_b[0] = '\0';
        item.text_c[0] = '\0';
        item.text_d[0] = '\0';
        item.text_e[0] = '\0';
        item.text_f[0] = '\0';

        if (vip_flag) {
            snprintf(item.text_a, sizeof(item.text_a), "0");
        } else{
            snprintf(item.text_a, sizeof(item.text_a), "6");
            item.color_a = 0x7e7e7e;
        }

        if (mv_flag) {
            snprintf(item.text_b, sizeof(item.text_b), "7");
            item.color_b = 0xa600ff;
        }

        snprintf(item.text_c, sizeof(item.text_c), "2");
        snprintf(item.text_d, sizeof(item.text_d), "3");
        snprintf(item.text_e, sizeof(item.text_e), "4");


        
        (void)lv_rich_song_catalog_set_item_by_slot(&adapter->catalog, item.slot_id, &item);
    }

    if (meta != NULL) {
        uint32_t received = (array_size > 0) ? (uint32_t)array_size : 0U;
        if (received < adapter->batch_size) {
            meta->reached_end = 1U;
            meta->final_total_count = start_id + received;
        }
    }

    cJSON_Delete(root);
    return 0;
}

static uint32_t rr_get_count(void *user_ctx)
{
    lv_rich_rank_adapter_t *adapter = (lv_rich_rank_adapter_t *)user_ctx;
    return (adapter != NULL) ? lv_rich_song_catalog_count(&adapter->catalog) : 0U;
}

static void rr_request_batch(lv_rich_rank_adapter_t *adapter, uint32_t batch_index)
{
    if (adapter == NULL || batch_index >= adapter->batch_count) {
        return;
    }

    if (adapter->end_reached && batch_index > adapter->last_batch_index) {
        printf("[rich_rank] 跳过请求: batch=%u 已超过最后一页 batch=%u\n",
               batch_index,
               adapter->last_batch_index);
        return;
    }

    if (adapter->batch_ready == NULL || adapter->batch_loading == NULL) {
        return;
    }

    if (adapter->batch_ready[batch_index] || adapter->batch_loading[batch_index]) {
        return;
    }

    adapter->batch_loading[batch_index] = 1U;
    fetch_song_by_rank_list_4_15((int)batch_index);
}

static bool rr_get_item(uint32_t index, lv_vlist_item_t *out, void *user_ctx)
{
    lv_rich_rank_adapter_t *adapter = (lv_rich_rank_adapter_t *)user_ctx;
    uint32_t batch_index;

    if (adapter == NULL || out == NULL) return false;

    batch_index = index / adapter->batch_size;
    rr_request_batch(adapter, batch_index);

    return lv_rich_song_catalog_get_vlist_item(&adapter->catalog, index, out);
}

static void rr_on_item_click(void *user_ctx,
                             uint32_t item_id,
                             uint32_t bound_index,
                             lv_coord_t rel_x,
                             lv_coord_t rel_y)
{
    lv_rich_rank_adapter_t *adapter = (lv_rich_rank_adapter_t *)user_ctx;
    lv_rich_song_item_t item;
    int item_changed = 0;

    (void)bound_index;
    (void)rel_y;

    if (adapter == NULL || adapter->view_style == NULL) {
        return;
    }

    if (!lv_rich_song_catalog_get_item(&adapter->catalog, item_id, &item)) {
        return;
    }

    lv_rich_song_row_style_t  *s = &adapter->view_style->row_style;

    extern int g_song_id;
    g_song_id = item.song_id;
    record_last_artist(item.artist);
    record_last_song_name(item.name);

    if (rel_x >= s->idx_x    && rel_x < (s->idx_x + s->idx_w))
    {
    }
    if (rel_x >= s->name_x   && rel_x < (s->name_x + s->name_w))
    {
    }
    if (rel_x >= s->artist_x && rel_x < (s->artist_x + s->artist_w))
    {
    }
    if (rel_x >= s->a_x      && rel_x < (s->a_x + s->a_w))
    {
    }
    if (rel_x >= s->b_x      && rel_x < (s->b_x + s->b_w))
    {
        page_nav_push("screen_102",&guider_ui.screen_102,&guider_ui.screen_102_del,setup_scr_screen_102);
    }
    if (rel_x >= s->c_x      && rel_x < (s->c_x + s->c_w))
    {
        snprintf(item.text_c, sizeof(item.text_c), "8");
        item.color_c = 0xFF0000;   /* 红色 */
        item_changed = 1;
        db_list_pro_thread_send_to_worker(DBP_WORKER_MSG_love_song, 0, 0, item.artist, item.name, NULL);  
    }
    if (rel_x >= s->d_x      && rel_x < (s->d_x + s->d_w))
    {
        page_nav_push("screen_2",&guider_ui.screen_2,&guider_ui.screen_2_del,setup_scr_screen_2);
        ktv_cloud_order_resolved_t resolved;
        char current_json[8192];
        char songinfo_json[16384];

        ktv_cloud_order_resolve_first_play_param("16666666666",
                                                "738ce11c448f23cd97b5e0a0ac901c4b",
                                                1,   // 或 1
                                                item.song_id,
                                                NULL,
                                                &resolved,
                                                current_json,
                                                sizeof(current_json),
                                                songinfo_json,
                                                sizeof(songinfo_json));
        ktv_player_ui_play(&resolved.play_param);

    }
    if (rel_x >= s->e_x      && rel_x < (s->e_x + s->e_w))
    {
        snprintf(item.text_e, sizeof(item.text_e), "9");
        item_changed = 1;

        db_list_pro_thread_send_to_worker(DBP_WORKER_MSG_add_song, 0, 0, item.artist, item.name, NULL); 
        set_order_num(6);
        char str1[100];
        snprintf(str1, sizeof(str1), "%d", get_order_num());
        lv_label_set_text(lv_obj_get_child(lv_obj_get_child(lv_scr_act(), 1), -1), str1);
    }
    if (rel_x >= s->f_x      && rel_x < (s->f_x + s->f_w))
    {

    }
    if (item_changed != 0)
    {
        (void)lv_rich_song_catalog_set_item_by_slot(&adapter->catalog, item_id, &item);
        if (adapter->vlist != NULL)
        {
            lv_vlist_notify_item_changed(adapter->vlist, item_id);
        }
    }
}

static int rr_pop_one_batch(lv_rich_rank_adapter_t *adapter,
                            uint32_t *batch_index,
                            uint8_t *success,
                            uint32_t *request_epoch)
{
    uint32_t i;

    if (adapter == NULL) return 0;

    pthread_mutex_lock(&adapter->pending_lock);
    for (i = 0U; i < adapter->batch_count; ++i) {
        if (adapter->pending_batch_valid[i]) {
            adapter->pending_batch_valid[i] = 0U;
            *batch_index = i;
            *success = adapter->pending_batch_success[i];
            *request_epoch = (adapter->pending_batch_epoch != NULL)
                           ? adapter->pending_batch_epoch[i]
                           : adapter->request_epoch;
            pthread_mutex_unlock(&adapter->pending_lock);
            return 1;
        }
    }

    adapter->has_pending = 0U;
    pthread_mutex_unlock(&adapter->pending_lock);
    return 0;
}

static void rr_apply_last_page_if_needed(lv_rich_rank_adapter_t *adapter,
                                         uint32_t batch_index,
                                         const rr_batch_sync_meta_t *meta)
{
    uint32_t final_total;
    uint32_t final_batch_count;

    if (adapter == NULL || meta == NULL || !meta->reached_end) {
        return;
    }

    final_total = meta->final_total_count;
    if (final_total > adapter->total_count) {
        final_total = adapter->total_count;
    }

    adapter->end_reached = 1U;
    adapter->last_batch_index = batch_index;

    if (final_total == adapter->total_count) {
        printf("[rich_rank] 已到最后一页: batch=%u, recv=%u, batch_size=%u, final_total=%u\n",
               batch_index,
               meta->data_count,
               adapter->batch_size,
               final_total);
        return;
    }

    adapter->total_count = final_total;
    final_batch_count = (final_total == 0U)
                      ? 0U
                      : ((final_total + adapter->batch_size - 1U) / adapter->batch_size);
    adapter->batch_count = final_batch_count;

    (void)lv_rich_song_catalog_truncate(&adapter->catalog, final_total);

    printf("[rich_rank] 已到最后一页: batch=%u, recv=%u, batch_size=%u, final_total=%u, json_total_count=%u%s\n",
           batch_index,
           meta->data_count,
           adapter->batch_size,
           final_total,
           meta->json_total_count,
           meta->json_total_count_valid ? "" : "(json未提供)");

    if (adapter->vlist != NULL) {
        lv_vlist_reload(adapter->vlist);
    }
}

static void rr_ui_timer_cb(lv_timer_t *timer)
{
    lv_rich_rank_adapter_t *adapter = (lv_rich_rank_adapter_t *)timer->user_data;
    uint32_t batch_index;
    uint32_t request_epoch;
    uint8_t success;

    if (adapter == NULL || adapter->vlist == NULL) return;

    pthread_mutex_lock(&adapter->pending_lock);
    if (!adapter->has_pending) {
        pthread_mutex_unlock(&adapter->pending_lock);
        return;
    }
    pthread_mutex_unlock(&adapter->pending_lock);

    while (rr_pop_one_batch(adapter, &batch_index, &success, &request_epoch)) {
        uint32_t start, count;
        rr_batch_sync_meta_t meta;

        if (request_epoch != adapter->request_epoch) {
            continue;
        }

        if (batch_index >= adapter->batch_count) {
            continue;
        }

        adapter->batch_loading[batch_index] = 0U;

        if (!success) continue;
        if (rr_sync_batch_from_json(adapter, batch_index, request_epoch, &meta) != 0) continue;

        rr_apply_total_count_hint_if_needed(adapter, &meta);
        rr_apply_last_page_if_needed(adapter, batch_index, &meta);

        if (batch_index >= adapter->batch_count) {
            continue;
        }

        adapter->batch_ready[batch_index] = 1U;
        start = batch_index * adapter->batch_size;
        count = adapter->batch_size;
        if (start >= adapter->total_count) {
            continue;
        }
        if (start + count > adapter->total_count) {
            count = adapter->total_count - start;
        }
        lv_vlist_notify_range_changed(adapter->vlist, start, count);
    }
}

int lv_rich_rank_adapter_start(lv_rich_rank_adapter_t *adapter,
                               const lv_rich_song_view_style_t *view_style,
                               uint32_t total_count,
                               uint32_t batch_size)
{
    if (adapter == NULL || view_style == NULL || total_count == 0U) {
        return -1;
    }

    memset(adapter, 0, sizeof(*adapter));
    adapter->view_style = view_style;
    adapter->total_count = total_count;
    adapter->batch_size = rr_safe_positive_u32(batch_size, 50U);
    adapter->batch_count = (total_count + adapter->batch_size - 1U) / adapter->batch_size;

    if (!lv_rich_song_catalog_init(&adapter->catalog, total_count)) {
        return -1;
    }

    adapter->batch_ready = (uint8_t *)calloc(adapter->batch_count, sizeof(uint8_t));
    adapter->batch_loading = (uint8_t *)calloc(adapter->batch_count, sizeof(uint8_t));
    adapter->pending_batch_valid = (uint8_t *)calloc(adapter->batch_count, sizeof(uint8_t));
    adapter->pending_batch_success = (uint8_t *)calloc(adapter->batch_count, sizeof(uint8_t));
    adapter->pending_batch_epoch = (uint32_t *)calloc(adapter->batch_count, sizeof(uint32_t));

    if (adapter->batch_ready == NULL ||
        adapter->batch_loading == NULL ||
        adapter->pending_batch_valid == NULL ||
        adapter->pending_batch_success == NULL ||
        adapter->pending_batch_epoch == NULL) {
        lv_rich_rank_adapter_stop(adapter);
        return -1;
    }

    pthread_mutex_init(&adapter->pending_lock, NULL);
    adapter->ui_timer = lv_timer_create(rr_ui_timer_cb, 50U, adapter);
    adapter->started = 1U;
    adapter->request_epoch = 1U;
    adapter->end_reached = 0U;
    adapter->last_batch_index = (adapter->batch_count > 0U) ? (adapter->batch_count - 1U) : 0U;
    g_rich_rank_default = adapter;
    return 0;
}

void lv_rich_rank_adapter_stop(lv_rich_rank_adapter_t *adapter)
{
    if (adapter == NULL) {
        return;
    }

    if (adapter->ui_timer != NULL) {
        lv_timer_del(adapter->ui_timer);
        adapter->ui_timer = NULL;
    }

    if (adapter->started) {
        pthread_mutex_destroy(&adapter->pending_lock);
    }

    free(adapter->batch_ready);
    free(adapter->batch_loading);
    free(adapter->pending_batch_valid);
    free(adapter->pending_batch_success);
    free(adapter->pending_batch_epoch);

    adapter->batch_ready = NULL;
    adapter->batch_loading = NULL;
    adapter->pending_batch_valid = NULL;
    adapter->pending_batch_success = NULL;
    adapter->pending_batch_epoch = NULL;

    lv_rich_song_catalog_deinit(&adapter->catalog);

    if (g_rich_rank_default == adapter) {
        g_rich_rank_default = NULL;
    }

    memset(adapter, 0, sizeof(*adapter));
}

lv_vlist_t *lv_rich_rank_adapter_create_vlist(lv_rich_rank_adapter_t *adapter, lv_obj_t *parent)
{
    lv_vlist_config_t cfg;
    lv_vlist_ops_t ops;

    if (adapter == NULL || parent == NULL || adapter->view_style == NULL) {
        return NULL;
    }

    memset(&cfg, 0, sizeof(cfg));
    cfg.parent = parent;
    cfg.visible_rows = rr_safe_positive_u32(adapter->view_style->visible_rows, 1U);
    cfg.visible_cols = rr_safe_positive_u32(adapter->view_style->visible_cols, 1U);
    cfg.overscan_rows_front = adapter->view_style->overscan_rows_front;
    cfg.overscan_rows_back = adapter->view_style->overscan_rows_back;
    cfg.preload_before = adapter->view_style->preload_before;
    cfg.preload_after = adapter->view_style->preload_after;
    cfg.viewport_width = adapter->view_style->viewport_width;
    cfg.viewport_height = adapter->view_style->viewport_height;
    cfg.cell_width = adapter->view_style->cell_width;
    cfg.cell_height = adapter->view_style->cell_height;
    cfg.gap_x = adapter->view_style->gap_x;
    cfg.gap_y = adapter->view_style->gap_y;
    cfg.user_ctx = adapter;
    cfg.renderer_ops = &g_lv_renderer_rich_song_ops;
    cfg.renderer_style = &adapter->view_style->row_style;
    cfg.on_item_click = rr_on_item_click;

    memset(&ops, 0, sizeof(ops));
    ops.get_count = rr_get_count;
    ops.get_item = rr_get_item;

    adapter->vlist = lv_vlist_create(&cfg, &ops);

    if (adapter->vlist != NULL) {
        rr_request_batch(adapter, 0U);
    }

    return adapter->vlist;
}

void lv_rich_rank_adapter_reset(lv_rich_rank_adapter_t *adapter, uint32_t total_count)
{
    if (adapter == NULL || total_count == 0U) {
        return;
    }

    pthread_mutex_lock(&adapter->pending_lock);
    adapter->request_epoch += 1U;
    if (adapter->request_epoch == 0U) {
        adapter->request_epoch = 1U;
    }
    adapter->has_pending = 0U;
    pthread_mutex_unlock(&adapter->pending_lock);

    adapter->total_count = total_count;
    adapter->batch_count = (total_count + adapter->batch_size - 1U) / adapter->batch_size;
    adapter->end_reached = 0U;
    adapter->last_batch_index = (adapter->batch_count > 0U) ? (adapter->batch_count - 1U) : 0U;

    (void)lv_rich_song_catalog_reset(&adapter->catalog, total_count);

    free(adapter->batch_ready);
    free(adapter->batch_loading);
    free(adapter->pending_batch_valid);
    free(adapter->pending_batch_success);
    free(adapter->pending_batch_epoch);

    adapter->batch_ready = (uint8_t *)calloc(adapter->batch_count, sizeof(uint8_t));
    adapter->batch_loading = (uint8_t *)calloc(adapter->batch_count, sizeof(uint8_t));
    adapter->pending_batch_valid = (uint8_t *)calloc(adapter->batch_count, sizeof(uint8_t));
    adapter->pending_batch_success = (uint8_t *)calloc(adapter->batch_count, sizeof(uint8_t));
    adapter->pending_batch_epoch = (uint32_t *)calloc(adapter->batch_count, sizeof(uint32_t));

    if (adapter->vlist != NULL) {
        lv_vlist_reload(adapter->vlist);
    }

    if (adapter->batch_ready == NULL ||
        adapter->batch_loading == NULL ||
        adapter->pending_batch_valid == NULL ||
        adapter->pending_batch_success == NULL ||
        adapter->pending_batch_epoch == NULL) {
        return;
    }

    rr_request_batch(adapter, 0U);
}

bool lv_rich_rank_adapter_get_business_item(lv_rich_rank_adapter_t *adapter,
                                            uint32_t item_id,
                                            lv_rich_song_item_t *out)
{
    if (adapter == NULL) {
        return false;
    }
    return lv_rich_song_catalog_get_item(&adapter->catalog, item_id, out);
}

void lv_rich_rank_adapter_toggle_selected(lv_rich_rank_adapter_t *adapter, uint32_t item_id)
{
    lv_rich_song_item_t item;

    if (adapter == NULL) {
        return;
    }

    if (!lv_rich_song_catalog_get_item(&adapter->catalog, item_id, &item)) {
        return;
    }

    lv_rich_song_catalog_set_selected(&adapter->catalog, item_id, !item.selected);
    if (adapter->vlist != NULL) {
        lv_vlist_notify_item_changed(adapter->vlist, item_id);
    }
}

void lv_rich_rank_adapter_notify_batch_ready_with_epoch(lv_rich_rank_adapter_t *adapter,
                                                        uint32_t batch_index,
                                                        bool success,
                                                        uint32_t request_epoch)
{
    if (adapter == NULL || batch_index >= adapter->batch_count) {
        return;
    }

    pthread_mutex_lock(&adapter->pending_lock);
    adapter->pending_batch_valid[batch_index] = 1U;
    adapter->pending_batch_success[batch_index] = success ? 1U : 0U;
    if (adapter->pending_batch_epoch != NULL) {
        adapter->pending_batch_epoch[batch_index] = request_epoch;
    }
    adapter->has_pending = 1U;
    pthread_mutex_unlock(&adapter->pending_lock);
}

lv_rich_rank_adapter_t *lv_rich_rank_adapter_get_default(void)
{
    return g_rich_rank_default;
}

uint32_t lv_rich_rank_adapter_get_request_epoch(const lv_rich_rank_adapter_t *adapter)
{
    return (adapter != NULL) ? adapter->request_epoch : 0U;
}

uint32_t lv_rich_rank_adapter_get_default_request_epoch(void)
{
    return lv_rich_rank_adapter_get_request_epoch(g_rich_rank_default);
}

void lv_rich_rank_adapter_notify_default_batch_ready_with_epoch(uint32_t batch_index,
                                                                bool success,
                                                                uint32_t request_epoch)
{
    lv_rich_rank_adapter_notify_batch_ready_with_epoch(g_rich_rank_default,
                                                       batch_index,
                                                       success,
                                                       request_epoch);
}
