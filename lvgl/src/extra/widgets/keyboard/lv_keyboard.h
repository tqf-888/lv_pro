...
/** Keyboard layout set.
 * Default keeps LVGL original QWERTY behavior.
 * AZ is the simplified A-Z/0-9 layout, enabled explicitly by lv_keyboard_set_layout().
 */
enum {
    LV_KEYBOARD_LAYOUT_QWERTY = 0,
    LV_KEYBOARD_LAYOUT_AZ,
    LV_KEYBOARD_LAYOUT_DEFAULT = LV_KEYBOARD_LAYOUT_QWERTY,
};
typedef uint8_t lv_keyboard_layout_t;

/* Compatibility aliases. */
#define LV_KEYBOARD_LAYOUT_ORIGINAL  LV_KEYBOARD_LAYOUT_QWERTY
#define LV_KEYBOARD_LAYOUT_AZ09      LV_KEYBOARD_LAYOUT_AZ
...
typedef struct {
    ...
    lv_keyboard_layout_t layout;    /*Keyboard layout set*/
...
/**
 * Set keyboard layout and reset to text lower mode.
 * @param kb pointer to a Keyboard object
 * @param layout layout from 'lv_keyboard_layout_t'
 */
void lv_keyboard_set_layout(lv_obj_t * kb, lv_keyboard_layout_t layout);

/**
 * Set keyboard layout and mode in one call.
 * @param kb pointer to a Keyboard object
 * @param layout layout from 'lv_keyboard_layout_t'
 * @param mode mode from 'lv_keyboard_mode_t'
 */
void lv_keyboard_set_layout_mode(lv_obj_t * kb, lv_keyboard_layout_t layout, lv_keyboard_mode_t mode);
...
/**
 * Get current keyboard layout set.
 * @param kb pointer to a Keyboard object
 * @return current layout from 'lv_keyboard_layout_t'
 */
lv_keyboard_layout_t lv_keyboard_get_layout(const lv_obj_t * kb);
